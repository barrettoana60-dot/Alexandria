...
import requests
import PyPDF2 # Assumindo que PyPDF2 está instalado ou PyPDF2 = None
import openpyxl # Assumindo que openpyxl está instalado ou openpyxl = None
import pandas as pd
import cv2 # OpenCV para processamento de imagem rápido

# ML / Image Processing — all optional, pure numpy fallbacks
# Com a adição de CV2, skimage e scipy se tornam menos críticos para as funções de imagem
SKIMAGE_OK = False
SKLEARN_OK = False
SCIPY_OK = False
...
try:
    ...
    pass # Não é mais crítico com CV2
...
st.set_page_config(page_title="Nebula AI 🔬", page_icon="🔬", layout="wide", initial_sidebar_state="expanded")
...
# --- Gradientes de Cores para Avatares (Ajustados para o tema azul) ---
GRAD_POOL=[
    "135deg,#00C9A7,#4CC9F0","135deg,#B17DFF,#6C63FF","135deg,#FFD60A,#FF8C42",
    "135deg,#06D6A0,#7BD3FF","135deg,#48C6EF,#00C9A7","135deg,#FF4E8A,#FF9A44",
    "135deg,#FF6B35,#F7C948","135deg,#845EC2,#B17DFF",
]
...
def call_claude_vision(img_bytes, prompt, api_key):
    ...
    try:
        ...
        resp = requests.post(
            ...
            json={
                "model": "claude-sonnet-4-20250514", # Usando um modelo mais recente
                "max_tokens": 1200,
                "messages": [{
                    "role": "user",
                    "content": [
...
# ════════════════════════════════════════════════
#  REAL ML IMAGE ANALYSIS PIPELINE (OTIMIZADO COM CV2)
# ════════════════════════════════════════════════
def sobel_analysis(gray_arr):
    """Multi-directional Sobel edge detection using OpenCV."""
    try:
        # Convert to uint8 if not already
        gray_u8 = cv2.normalize(gray_arr, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        ...
        # Fallback to numpy if OpenCV fails
...
def canny_analysis(gray_uint8):
    """Canny multi-scale edge detection using OpenCV."""
    try:
        # Using fixed thresholds for simplicity, can be adaptive
        edges_fine = cv2.Canny(gray_uint8, 50, 150)
        edges_med = cv2.Canny(gray_uint8, 70, 200)
        edges_coarse = cv2.Canny(gray_uint8, 90, 250)

        # Convert to boolean arrays for consistency with skimage output
        edges_fine_bool = edges_fine > 0
        edges_med_bool = edges_med > 0
        edges_coarse_bool = edges_coarse > 0

        return {
            "fine": edges_fine_bool, "medium": edges_med_bool, "coarse": edges_coarse_bool,
            "fine_density": float(edges_fine_bool.mean()),
            "medium_density": float(edges_med_bool.mean()),
            "coarse_density": float(edges_coarse_bool.mean()),
            "total_edges": int(edges_fine_bool.sum()),
            "structure_level": "micro" if edges_fine_bool.mean()>0.1 else ("meso" if edges_med_bool.mean()>0.05 else "macro")
        }
    except Exception:
        # Fallback to numpy gradient-based edge approximation
        ...
        return {"fine":edges_fine,"medium":edges_med,"coarse":edges_coarse,"fine_density":float(edges_fine.mean()),
                "medium_density":float(edges_med.mean()),"coarse_density":float(edges_coarse.mean()),
                "total_edges":int(edges_fine.sum()),"structure_level":"meso"}

def orb_keypoints(gray_uint8):
    """ORB feature detection using OpenCV."""
    try:
        orb = cv2.ORB_create(nfeatures=500, scaleFactor=1.2, nlevels=8, edgeThreshold=31, firstLevel=0, WTA_K=2, scoreType=cv2.ORB_HARRIS_SCORE, patchSize=31)
        ...
        mean_scale = float(scales.mean()) if len(scales) > 0 else 1.0

        centers = []
        if len(kp_coords) > 0 and SKLEARN_OK:
            n_cl = min(5, len(kp_coords))
            try:
                kmk = KMeans(n_clusters=n_cl, random_state=42, n_init=5).fit(kp_coords)
                centers = kmk.cluster_centers_
            except Exception:
                centers = kp_coords[:n_cl] # Fallback if KMeans fails
        else:
            centers = kp_coords[:min(5, len(kp_coords))]

        distribution = "uniforme"
        if len(kp_coords) > 5:
            # Simple check for concentration
            std_x = np.std(kp_coords[:,0])
            std_y = np.std(kp_coords[:,1])
            if std_x / (std_y + 1e-5) > 2 or std_y / (std_x + 1e-5) > 2:
                distribution = "concentrado"
            elif np.mean(np.linalg.norm(kp_coords - np.mean(kp_coords, axis=0), axis=1)) < np.mean(gray_uint8.shape) / 8:
                distribution = "centralizado"

        return {
            "keypoints": kp_coords.tolist(),
        ...
        return {"keypoints":[],"n_keypoints":0,"cluster_centers":[],"scales":[],"mean_scale":1.0,"distribution":"n/a"}

def glcm_texture(gray_uint8):
    """GLCM texture using scikit-image (if available) or numpy fallback."""
    try:
        if SKIMAGE_OK:
            from skimage.feature import graycomatrix, graycoprops
            # Reduce levels for faster computation and robustness
            g_quantized = (gray_uint8 // 16).astype(np.uint8) # 16 levels
            distances=[1, 3]; angles=[0, np.pi/4, np.pi/2, 3*np.pi/4]
            glcm = graycomatrix(g_quantized, distances=distances, angles=angles, levels=16, symmetric=True, normed=True)
            ...
            # Numpy-based texture statistics (simplified for speed)
            ...
            energy=float(np.var(g)) # Local variance as energy proxy
...
def kmeans_colors(img_arr, k=7):
    ...
    try:
        h, w = img_arr.shape[:2]
        # Sample pixels for speed (more aggressive sampling)
        step = max(1, (h*w) // 2000) # Reduced sample size for speed
...
def fft_analysis(gray_arr):
    ...
    try:
        ...
        magnitude_mean = np.mean(magnitude) + 1e-5

        # Define regions for periodic score
        center_region = magnitude[h//2-30:h//2+30, w//2-30:w//2+30]

        # Ensure outer region is not empty
        outer_top = magnitude[:h//4, :].ravel()
        outer_bottom = magnitude[3*h//4:, :].ravel()
        outer_left = magnitude[:, :w//4].ravel()
        outer_right = magnitude[:, 3*w//4:].ravel()

        outer = np.concatenate([outer_top, outer_bottom, outer_left, outer_right])

        if len(outer) == 0: # Fallback if image is too small
            periodic_score = 1.0
        else:
            periodic_score = float(np.percentile(outer,99)) / magnitude_mean
        ...
        # print(f"FFT analysis error: {e}") # Debugging
        return {
            "periodic_score": 1.0,
            "is_periodic": False,
            "low_freq": 0.333,
...
def run_full_ml_pipeline(img_bytes):
    ...
    result = {}
    try:
        # Usar OpenCV para carregar e redimensionar para maior velocidade
        ...
        orig_size = (img_cv.shape[1], img_cv.shape[0]) # (width, height)

        # Redimensionar para processamento (mantendo proporção)
        h_orig, w_orig = img_cv.shape[:2]
        scale = min(512 / w_orig, 512 / h_orig)
        new_w, new_h = int(w_orig * scale), int(h_orig * scale)
        img_resized = cv2.resize(img_cv, (new_w, new_h), interpolation=cv2.INTER_AREA)

        arr = img_resized.astype(np.float32) # RGB float array

        # Converter para escala de cinza
        gray = cv2.cvtColor(img_resized, cv2.COLOR_BGR2GRAY)
        gray_u8 = gray.astype(np.uint8)

        # Canais de cor (BGR em OpenCV)
        b_ch, g_ch, r_ch = cv2.split(img_resized)
        ...
        sym = 1.0 - (max(q)-min(q)) / (max(q)+1e-5) if max(q) > 1e-5 else 1.0

        # Entropy
        hist_gray = cv2.calcHist([gray_u8], [0], None, [64], [0, 256]).flatten()
        hn = hist_gray / hist_gray.sum()
        hn = hn[hn > 0]
        ...
        result["sobel"] = sobel_analysis(gray) # Pass gray directly
        result["canny"] = canny_analysis(gray_u8)
        result["orb"]   = orb_keypoints(gray_u8)
        result["glcm"]  = glcm_texture(gray_u8)

        result["fft"] = fft_analysis(gray) # Pass gray directly
        result["kmeans_palette"], result["color_temps"] = kmeans_colors(img_resized, k=7) # Pass resized color image

        # Histograms
        rh = cv2.calcHist([r_ch], [0], None, [32], [0, 256]).flatten().tolist()
        gh = cv2.calcHist([g_ch], [0], None, [32], [0, 256]).flatten().tolist()
        bh = cv2.calcHist([b_ch], [0], None, [32], [0, 256]).flatten().tolist()
        ...
        if "magnitude" in result["sobel"]:
            ...
            # Normalize to 0-255 and convert to uint8 for visualization
            result["sobel_viz"] = cv2.normalize(mag_norm, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8).tolist()
        result["array_shape"] = [new_h, new_w]
        result["ok"] = True
    except Exception as e:
        result["ok"] = False
        result["error"] = str(e)
        # st.error(f"Erro no pipeline ML: {e}") # Para depuração
    return result
...
def extract_pdf(b):
    ...
    try:
        reader=PyPDF2.PdfReader(io.BytesIO(b)); t=""
        for pg in reader.pages[:20]: # Limita a 20 páginas para performance
            try: t+=pg.extract_text()+"\n"
            except: pass
        return t[:40000] # Limita o texto extraído
    except: return ""
...
EMAP={"pdf":"PDF","docx":"Word","xlsx":"Planilha","csv":"Dados","txt":"Texto","py":"Código","md":"Markdown","png":"Imagem","jpg":"Imagem","jpeg":"Imagem","webp":"Imagem","tiff":"Imagem"}
def ftype(fname): return EMAP.get(fname.split(".")[-1].lower() if "." in fname else "","Arquivo")

# --- Cores da Interface (Ajustadas para o tema azul escuro) ---
VIB=["#06D6A0","#4CC9F0","#B17DFF","#FFD60A","#FF8C42","#FF3B5C","#7BD3FF","#00C9A7","#FFAB00","#845EC2"]

# ═══════════════════════════════════════════════════
#  CSS (TEMA AZUL ESCURO)
# ═══════════════════════════════════════════════════
def inject_css():
    st.markdown("""
<style>
...
:root {
  --bg:#0A0E17; /* Fundo principal: Azul muito escuro */
  --bg2:#0E142A; /* Fundo secundário: Azul escuro */
  --bg3:#151D36; /* Fundo terciário: Azul médio-escuro */

  --yel:#FFD60A; --yel2:#FFEC47; /* Amarelo */
  --grn:#06D6A0; --grn2:#00F5C4; /* Verde */
  --red:#FF3B5C; --red2:#FF6B81; /* Vermelho */
  --blu:#4CC9F0; --blu2:#7BD3FF; /* Azul claro */
  --pur:#B17DFF; --orn:#FF8C42; /* Roxo e Laranja */

  --t0:#FFFFFF; --t1:#E8E9F0; --t2:#A8ABBE; --t3:#6B6F88; --t4:#404460; /* Tons de texto */

  --g1:rgba(255,255,255,.06); --g2:rgba(255,255,255,.09); --g3:rgba(255,255,255,.13); /* Gradientes de branco */
  --gb1:rgba(255,255,255,.08); --gb2:rgba(255,255,255,.14); --gb3:rgba(255,255,255,.22); /* Bordas de branco */

  --r8:8px; --r12:12px; --r16:16px; --r20:20px; --r28:28px; /* Raio das bordas */
}
...
.stApp::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 60% 50% at -5% 0%,rgba(76,201,240,.07) 0%,transparent 60%),radial-gradient(ellipse 50% 40% at 105% 0%,rgba(177,125,255,.07) 0%,transparent 55%),radial-gradient(ellipse 40% 50% at 50% 110%,rgba(6,214,160,.06) 0%,transparent 60%);}
...
section[data-testid="stSidebar"]{
  ...
  background:var(--bg2)!important; /* Fundo da sidebar */
...
.stButton>button{
  background:var(--bg3)!important; /* Fundo dos botões */
...
.stButton>button:hover{
  background:var(--bg)!important; /* Fundo dos botões ao passar o mouse */
...
.sb-logo-icon{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,var(--blu),var(--pur));display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0;}
.sb-logo-txt{font-family:'Syne',sans-serif;font-weight:900;font-size:1.2rem;letter-spacing:-.04em;background:linear-gradient(135deg,var(--blu),var(--grn));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.sb-lbl{font-size:.54rem;font-weight:700;color:var(--t4);letter-spacing:.14em;text-transform:uppercase;padding:0 .2rem;margin-bottom:.35rem;margin-top:.8rem;}
.stTextInput input,.stTextArea textarea{background:var(--bg3)!important;border:1px solid var(--gb1)!important;border-radius:var(--r12)!important;color:var(--t1)!important;font-family:'DM Sans',sans-serif!important;font-size:.84rem!important;}
.stTextInput input:focus,.stTextArea textarea:focus{border-color:var(--blu2)!important;box-shadow:0 0 0 3px rgba(76,201,240,.08)!important;}
...
.glass{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r20);box-shadow:0 0 0 1px rgba(255,255,255,.04) inset,0 4px 32px rgba(0,0,0,.3);position:relative;overflow:hidden;}
...
.post-card{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r20);margin-bottom:.65rem;overflow:hidden;box-shadow:0 2px 20px rgba(0,0,0,.25);transition:border-color .14s,transform .14s;}
.post-card:hover{border-color:var(--blu2);transform:translateY(-1px);}
.sc{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r20);padding:.9rem 1rem;margin-bottom:.6rem;}
.scard{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r16);padding:.8rem 1rem;margin-bottom:.42rem;transition:border-color .13s,transform .13s;}
.scard:hover{border-color:var(--blu2);transform:translateY(-1px);}
.mbox{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r16);padding:.9rem;text-align:center;}
.abox{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r16);padding:1rem;margin-bottom:.6rem;}
...
.chart-wrap{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r12);padding:.65rem;margin-bottom:.6rem;}
.compose-box{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r20);padding:1.1rem 1.3rem;margin-bottom:.8rem;}
...
.tag{display:inline-block;background:var(--bg3);border:1px solid var(--gb1);border-radius:50px;padding:2px 9px;font-size:.63rem;color:var(--t2);margin:2px;font-weight:500;}
...
.bme{background:linear-gradient(135deg,rgba(76,201,240,.15),rgba(177,125,255,.1));border:1px solid rgba(76,201,240,.2);border-radius:18px 18px 4px 18px;padding:.55rem .88rem;max-width:70%;margin-left:auto;margin-bottom:5px;font-size:.82rem;line-height:1.6;}
.bthem{background:var(--bg3);border:1px solid var(--gb1);border-radius:18px 18px 18px 4px;padding:.55rem .88rem;max-width:70%;margin-bottom:5px;font-size:.82rem;line-height:1.6;}
.cmt{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r12);padding:.5rem .85rem;margin-bottom:.25rem;}
.stTabs [data-baseweb="tab-list"]{background:var(--bg3)!important;border:1px solid var(--gb1)!important;border-radius:var(--r12)!important;padding:3px!important;gap:2px!important;}
...
.stTabs [aria-selected="true"]{background:var(--bg)!important;color:var(--blu)!important;border:1px solid rgba(76,201,240,.2)!important;font-weight:700!important;}
.stTabs [data-baseweb="tab-panel"]{background:transparent!important;padding-top:.8rem!important;}
.prof-hero{background:var(--bg3);backdrop-filter:blur(32px);border:1px solid var(--gb1);border-radius:var(--r28);padding:1.6rem;display:flex;gap:1.2rem;align-items:flex-start;box-shadow:0 6px 40px rgba(0,0,0,.35);margin-bottom:1rem;}
...
.stAlert{background:var(--bg3)!important;border:1px solid var(--gb1)!important;border-radius:var(--r16)!important;}
.stSelectbox [data-baseweb="select"]{background:var(--bg3)!important;border:1px solid var(--gb1)!important;border-radius:var(--r12)!important;}
.stFileUploader section{background:var(--bg3)!important;border:1.5px dashed var(--gb2)!important;border-radius:var(--r16)!important;}
.stExpander{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r16);}
...
.stRadio>div>label{background:var(--bg3)!important;border:1px solid var(--gb1)!important;border-radius:50px!important;padding:.28rem .78rem!important;font-size:.74rem!important;cursor:pointer!important;color:var(--t2)!important;}
/* colored accent buttons */
input[type="number"]{background:var(--bg3)!important;border:1px solid var(--gb1)!important;border-radius:var(--r12)!important;color:var(--t1)!important;}
/* AI Result card */
.ai-card{background:linear-gradient(135deg,rgba(76,201,240,.06),rgba(177,125,255,.04));border:1px solid rgba(76,201,240,.18);border-radius:var(--r16);padding:1rem;margin-bottom:.6rem;}
.ml-feat{background:var(--bg3);border:1px solid var(--gb1);border-radius:var(--r12);padding:.65rem .85rem;margin-bottom:.38rem;}
...
def avh(initials,sz=40,grad=None):
    fs=max(sz//3,9); bg=grad or "linear-gradient(135deg,var(--blu),var(--pur))" # Ajustado para azul
...
def page_login():
  ...
  <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:.8rem">
    <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,var(--blu),var(--pur));display:flex;align-items:center;justify-content:center;font-size:1.4rem;box-shadow:0 0 24px rgba(76,201,240,.3)">🔬</div>
    <div style="font-family:Syne,sans-serif;font-size:2.6rem;font-weight:900;letter-spacing:-.06em;background:linear-gradient(135deg,var(--blu),var(--grn));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">Nebula</div>
  </div>
...
#  SIDEBAR
# ═══════════════════════════════════════════════
NAV=[("feed","🏠 Feed","blu"),("search","🔍 Busca","grn"),("knowledge","🕸 Conexões IA","pur"),
     ("folders","📁 Pastas","yel"),("analytics","📊 Análises","orn"),
     ("img_search","🔬 Visão IA","blu"),("chat","💬 Chat","grn"),("settings","⚙️ Config","red")]

def render_nav():
    """Simple sidebar nav — st.sidebar + plain st.button. Always works."""
    email = st.session_state.current_user
    u = guser(); name = u.get("name","?"); ini_ = ini(name)
    ...
    with st.sidebar:
        ...
        # Inject per-button active CSS — targets by nth-child, no :has() needed
        active_styles = ""
        for i, (key, label, col) in enumerate(NAV):
            is_a = (cur == key and not st.session_state.profile_view)
            if is_a:
                colors = {"yel":"var(--yel)","grn":"var(--grn)","blu":"var(--blu)",
                          "red":"var(--red)","pur":"var(--pur)","orn":"var(--orn)"}
                c = colors.get(col, "var(--blu)") # Default to blue
...
def page_feed():
    ...
    if st.session_state.compose_open:
        ...
        with st.form("new_post_form"):
            ...
            submit_post = st.form_submit_button("Publicar Pesquisa", use_container_width=True)
            if submit_post:
                if post_title and post_abstract:
                    new_id = max([p["id"] for p in st.session_state.feed_posts]) + 1 if st.session_state.feed_posts else 1
                    new_post = {
                        "id": new_id,
                        "author": guser().get("name", "Anônimo"),
                        "author_email": st.session_state.current_user,
                        "avatar": ini(guser().get("name", "Anônimo")),
                        "area": guser().get("area", "Geral"),
                        "title": post_title,
                        "abstract": post_abstract,
                        "tags": [t.strip() for t in post_tags_str.split(',') if t.strip()],
                        "likes": 0,
                        "comments": [],
                        "status": post_status,
                        ...
                        "liked_by": [],
                        "saved_by": [],
                        "connections": [], # Pode ser preenchido por IA ou manualmente
                        "views": 0
                    }
                    st.session_state.feed_posts.insert(0, new_post) # Adiciona no início
                    record(new_post["tags"], 1.0)
                    save_db()
    ...
    # Recommendations
    recs = get_recs(st.session_state.current_user, 3)
    if recs:
        st.markdown('<div style="font-size:.59rem;color:var(--blu);font-weight:700;margin-bottom:.4rem;letter-spacing:.10em;text-transform:uppercase">Recomendações para você</div>', unsafe_allow_html=True)
        for p in recs:
...
def page_search():
    ...
    if st.session_state.get("search_results") and st.session_state.get("last_sq"):
        ...
        with ta:
            if neb:
                st.markdown('<div style="font-size:.59rem;color:var(--blu);font-weight:700;margin-bottom:.4rem;letter-spacing:.10em;text-transform:uppercase">Na Nebula</div>', unsafe_allow_html=True)
...
def page_knowledge():
    ...
    for e1,e2,common,strength in edges:
        ...
        fig.add_trace(go.Scatter3d(x=[p1["x"],p2["x"],None],y=[p1["y"],p2["y"],None],z=[p1["z"],p2["z"],None],
            mode="lines",line=dict(color=f"rgba(76,201,240,{alpha:.2f})",width=min(3,1+strength)),hoverinfo="none",showlegend=False)) # Cor da linha ajustada
    nc=[("⭐ Você","var(--blu)") if ue==email else(("var(--grn)","var(--grn)") if ue in st.session_state.followed else ("var(--pur)","var(--pur)")) for ue in rlist] # Cores ajustadas
    ...
    fig.add_trace(go.Scatter3d(
        ...
        textfont=dict(color="var(--t3)",size=9,family="DM Sans"), # Cor do texto ajustada
    ...
    for col,(cls,v,l) in zip([c1,c2,c3,c4],[("mval-blu",len(rlist),"Pesquisadores"),("mval-grn",len(edges),"Conexões"),("mval-pur",len(st.session_state.followed),"Seguindo"),("mval-yel",len(st.session_state.feed_posts),"Pesquisas")]): # Cores ajustadas
        ...
        if not api_key or not api_key.startswith("sk-"):
            ...
            if ai_result:
                ...
                for sug in suggestions:
                    ...
                    score_col = "var(--grn)" if score>=80 else ("var(--yel)" if score>=60 else "var(--orn)") # Cores ajustadas
  ...
  <div style="display:flex;align-items:center;gap:9px;margin-bottom:.55rem">
    ...
    <div style="text-align:center;background:var(--bg3);border-radius:10px;padding:.38rem .65rem;flex-shrink:0">
  ...
  <div style="background:var(--bg3);border-radius:10px;padding:.55rem .75rem;margin-bottom:.5rem;font-size:.76rem;color:var(--t2);line-height:1.65;border:1px solid rgba(177,125,255,.10)">
            ...
            if analyses:
                for f,an in analyses.items():
                    with st.expander(f"🔬 {f}"):
                        ...
                        st.markdown(f'<div class="abox"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:.86rem;margin-bottom:.3rem">{f}</div><div style="font-size:.76rem;color:var(--t2)">{an.get("summary","")}</div><div style="display:flex;gap:1.2rem;margin-top:.5rem"><div><div style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:900;color:{rc}">{rel}%</div><div style="font-size:.55rem;color:var(--t3);text-transform:uppercase">Relevância</div></div><div><div style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:900;color:var(--blu)}">{wq}%</div><div style="font-size:.55rem;color:var(--t3);text-transform:uppercase">Qualidade</div></div><div><div style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:900;color:var(--orn)}">{an.get("word_count",0)}</div><div style="font-size:.55rem;color:var(--t3);text-transform:uppercase">Palavras</div></div></div></div>', unsafe_allow_html=True)
                        if kws:
                            st.markdown(tags_html(kws[:16]), unsafe_allow_html=True)
                        if topics:
                            fig2=go.Figure(go.Pie(labels=list(topics.keys()),values=list(topics.values()),hole=0.5,marker=dict(colors=VIB[:len(topics)],line=dict(color=["var(--bg0)"]*15,width=2)),textfont=dict(color="white",size=8)))
                            fig2.update_layout(height=220,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",legend=dict(font=dict(color="var(--t3)",size=8)),margin=dict(l=0,r=0,t=10,b=0))
...
def page_analytics():
    ...
    with tf:
        ...
        if not folders: st.markdown('<div class="glass" style="text-align:center;padding:3rem;color:var(--t3)">Crie pastas.</div>', unsafe_allow_html=True)
            ...
            if all_top:
                ...
                fig.update_layout(**{**pc_dark(),'height':250,'yaxis':dict(showgrid=False,color="var(--t3)",tickfont=dict(size=9)),'title':dict(text="Temas",font=dict(color="var(--t0)",family="Syne",size=11))})
    ...
    with tpr:
        ...
        if prefs:
            ...
            if len(cats)>=3:
                fig3=go.Figure(go.Scatterpolar(r=vals+[vals[0]],theta=cats+[cats[0]],fill='toself',line=dict(color="var(--blu)"),fillcolor="rgba(76,201,240,.10)")) # Cor ajustada
                fig3.update_layout(height=265,polar=dict(bgcolor="rgba(0,0,0,0)",radialaxis=dict(visible=True,gridcolor="rgba(255,255,255,.04)",color="var(--t3)",tickfont=dict(size=8)),angularaxis=dict(gridcolor="rgba(255,255,255,.04)",color="var(--t3)",tickfont=dict(size=9))),paper_bgcolor="rgba(0,0,0,0)",margin=dict(l=40,r=40,t=15,b=15))
...
def page_img_search():
    ...
    with cu:
        ...
        run=st.button("🔬 Analisar Imagem", key="btn_run", use_container_width=True)
        ...
        st.markdown('</div>', unsafe_allow_html=True) # Fechar o glass box
    ...
    with cr:
        if run and img_bytes:
            ...
            if not ml_result.get("ok"):
                ...
                # col_ = ml_result["color"] # Não usado diretamente aqui
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:.55rem">
    ...
    <div style="background:var(--bg3);border-radius:12px;padding:.55rem .9rem;text-align:center;flex-shrink:0;border:1px solid var(--gb1)">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:4px">
    {"".join(f'<span style="background:var(--bg3);border:1px solid var(--gb1);border-radius:20px;padding:2px 8px;font-size:.60rem;color:var(--t3)">{k}: {v}pt</span>' for k,v in cls_["all_scores"].items())}
  </div>
</div>''', unsafe_allow_html=True)
                ...
                with t1:
                    ...
                    fig_e=go.Figure(go.Bar(y=eh,marker=dict(color=list(range(16)),colorscale=[[0,"var(--bg3)"],[.3,"var(--red)"],[.7,"var(--yel)"],[1,"var(--grn)"]]),x=list(range(len(eh))))) # Cores ajustadas
                    fig_e.update_layout(**{**pc_dark(),'height':170,'title':dict(text="Distribuição de Intensidades Sobel",font=dict(color="var(--t0)",family="Syne",size=10)),'margin':dict(l=10,r=10,t=32,b=8)})
                ...
                with t2:
                    ...
                    # Keypoint scatter
                    kps=orb_r.get("keypoints",[])
                    if len(kps)>0:
                        kps_arr=np.array(kps)
                        if len(kps_arr.shape)==2 and kps_arr.shape[1]>=2:
                            h,w=ml_result.get("array_shape",[512,512])
                            fig_kp=go.Figure()
                            fig_kp.add_trace(go.Scatter(x=kps_arr[:,0],y=h-kps_arr[:,1],mode='markers', # Ajustado x,y para consistência
                            ...
                            if ctrs:
                                ca2=np.array(ctrs)
                                fig_kp.add_trace(go.Scatter(x=ca2[:,0],y=h-ca2[:,1], # Ajustado x,y para consistência
                                    mode='markers',marker=dict(size=12,symbol='x',color=VIB[2],line=dict(color='white',width=2)),name="Centros"))
                            fig_kp.update_layout(**{**pc_dark(),'height':250,'title':dict(text=f"Mapa de {n_kp} Keypoints ORB",font=dict(color="var(--t0)",family="Syne",size=10)),'xaxis':dict(range=[0,w],showgrid=False),'yaxis':dict(range=[0,h],showgrid=False,scaleanchor='x'),'margin':dict(l=10,r=10,t=32,b=8)})
                ...
                with t3:
                    ...
                    if glcm_props:
                        ...
                        fig_gl=go.Figure()
                        fig_gl.add_trace(go.Bar(x=names_g,y=vals_g,marker=dict(color=vals_norm,colorscale=[[0,"var(--bg3)"],[.4,"var(--blu)"],[.7,"var(--grn)"],[1,"var(--yel)"]]),text=[f"{v:.4f}" for v in vals_g],textposition="outside",textfont=dict(color="var(--t3)",size=8))) # Cores ajustadas
                        fig_gl.update_layout(**{**pc_dark(),'height':240,'title':dict(text="GLCM — Gray Level Co-occurrence Matrix",font=dict(color="var(--t0)",family="Syne",size=10)),'margin':dict(l=10,r=10,t=36,b=8)})
                    ...
                    st.markdown(f'<div class="ml-feat"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;font-weight:600;margin-bottom:.45rem">Textura: <span style="color:var(--yel)}">{tt}</span></div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;font-size:.72rem"><div style="color:var(--t2)">Contraste<br><strong>{contrast:.4f}</strong></div><div style="color:var(--t2)">Homog.<br><strong>{hom:.4f}</strong></div><div style="color:var(--t2)">Correlação<br><strong>{corr:.4f}</strong></div></div></div>', unsafe_allow_html=True)
                ...
                with t4:
                    ...
                    if pal:
                        ...
                        for ci,cp in enumerate(pal):
                            ...
                            st.markdown(f'<div style="display:flex;align-items:center;gap:9px;margin-bottom:.4rem"><div style="width:30px;height:30px;border-radius:7px;background:{hex_c};border:1.5px solid var(--gb1);flex-shrink:0"></div><div style="flex:1"><div style="display:flex;justify-content:space-between;font-size:.70rem;color:var(--t2);margin-bottom:2px"><span style="font-weight:600">{hex_c.upper()}</span><span>{pct:.1f}%</span></div>{bar}</div><div style="font-size:.60rem;color:var(--t3);width:80px;text-align:right">RGB({r},{g},{b})</div></div>', unsafe_allow_html=True)
                        # Pie chart
                        fig_pal=go.Figure(go.Pie(values=[c["pct"] for c in pal],labels=[c["hex"] for c in pal],
                            marker=dict(colors=[c["hex"] for c in pal],line=dict(color=["var(--bg0)"]*7,width=2)), # Cor ajustada
                        ...
                        fig_pal.update_layout(height=220,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
                            margin=dict(l=0,r=0,t=10,b=0),legend=dict(font=dict(color="var(--t3)",size=7)))
                ...
                with t5:
                    ...
                    fig_fft=go.Figure(go.Bar(x=["Baixa\n(estruturas grandes)","Média\n(detalhes)","Alta\n(ruído/textura fina)"],
                        y=[lf,mf,hf],marker=dict(color=[VIB[0],VIB[1],VIB[2]]),text=[f"{v:.3f}" for v in [lf,mf,hf]],textposition="outside",textfont=dict(color="var(--t3)",size=9))) # Cor ajustada
                    fig_fft.update_layout(**{**pc_dark(),'height':220,'title':dict(text="FFT — Distribuição de Frequências Espaciais",font=dict(color="var(--t0)",family="Syne",size=10)),'margin':dict(l=10,r=10,t=36,b=8)})
                    ...
                    st.markdown(f'<div class="ml-feat"><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;font-size:.72rem"><div><div style="color:var(--t2)">Score periódico<br><strong>{per:.1f}</strong></div></div><div><div style="color:var(--t2)">Escala dominante<br><strong>{dom}</strong></div></div><div><div style="color:var(--t2)">Estrutura<br><strong>{"Periódica ✓" if is_per else "Não-periódica"}</strong></div></div></div></div>', unsafe_allow_html=True)
                ...
                with t6:
                    ...
                    if h_data:
                        fig4=go.Figure()
                        fig4.add_trace(go.Scatter(x=bx,y=h_data.get("r",[])[:32],fill='tozeroy',name='R',line=dict(color='var(--red)',width=1.5),fillcolor='rgba(255,59,92,.10)')) # Cor ajustada
                        fig4.add_trace(go.Scatter(x=bx,y=h_data.get("g",[])[:32],fill='tozeroy',name='G',line=dict(color='var(--grn)',width=1.5),fillcolor='rgba(6,214,160,.10)')) # Cor ajustada
                        fig4.add_trace(go.Scatter(x=bx,y=h_data.get("b",[])[:32],fill='tozeroy',name='B',line=dict(color='var(--blu)',width=1.5),fillcolor='rgba(76,201,240,.10)')) # Cor ajustada
                        layout4={**pc_dark(),'height':200,'title':dict(text="Histograma RGB",font=dict(color="var(--t0)",family="Syne",size=10)),'margin':dict(l=10,r=10,t=32,b=8),'legend':dict(font=dict(color="var(--t3)",size=9))}
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:.8rem">
    ...
    <div style="background:var(--bg3);border-radius:12px;padding:.5rem .85rem;text-align:center;flex-shrink:0">
  ...
  <div style="background:var(--bg3);border-radius:10px;padding:.7rem .9rem;margin-bottom:.6rem;font-size:.78rem;color:var(--t2);line-height:1.7;border:1px solid var(--gb1)">
  ...
  {f'<div style="background:rgba(6,214,160,.05);border:1px solid rgba(6,214,160,.12);border-radius:8px;padding:.5rem .7rem;font-size:.72rem;color:var(--t2);line-height:1.65"><strong>💡 Observações:</strong> {obs}</div>' if obs else ""}
</div>''', unsafe_allow_html=True)
                    ...
                    st.markdown(f'<div class="abox"><div style="font-size:.62rem;color:var(--pur);font-weight:700;margin-bottom:.5rem">🤖 Resposta Bruta Claude Vision</div><div style="font-size:.78rem;color:var(--t2);line-height:1.7;white-space:pre-wrap">{ai_text[:1500]}</div></div>', unsafe_allow_html=True)
...
def page_chat():
    ...
    with cm:
        if st.session_state.active_chat:
            ...
            dot='<span class="dot-on"></span>' if online else '<span class="dot-off"></span>'
            st.markdown(f'<div style="background:var(--bg3);border:1px solid var(--gb1);border-radius:14px;padding:10px 14px;margin-bottom:.85rem;display:flex;align-items:center;gap:10px">{avh(ci,36,cg)}<div style="flex:1"><div style="font-weight:700;font-size:.88rem;font-family:Syne,sans-serif;color:var(--t0)">{dot}{cn}</div><div style="font-size:.63rem;color:var(--grn)">🔒 AES-256</div></div></div>', unsafe_allow_html=True)
            for msg in msgs:
...
def page_settings():
    ...
    st.markdown(f'<div class="abox"><div style="font-size:.58rem;color:var(--t3);text-transform:uppercase;letter-spacing:.10em;margin-bottom:.4rem;font-weight:700">Conta</div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:.95rem;color:var(--blu)}">{email}</div></div>', unsafe_allow_html=True) # Cor ajustada
    ...
    for nm,ds in [("🔒 AES-256","End-to-end"),("🔏 SHA-256","Hash senhas"),("🛡 TLS 1.3","Transmissão")]:
        st.markdown(f'<div class="pbox-grn"><div style="display:flex;align-items:center;gap:9px"><div><div style="font-weight:700;color:var(--grn);font-size:.78rem">{nm}</div><div style="font-size:.66rem;color:var(--t3)">{ds}</div></div></div></div>', unsafe_allow_html=True)
    st.markdown("<hr>", unsafe_allow_html=True)
