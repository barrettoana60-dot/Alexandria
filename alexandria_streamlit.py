...
def guser(): return st.session_state.users.get(st.session_state.current_user, {})
def get_photo(email): return st.session_state.users.get(email,{}).get("photo_b64")
...
def analyze_image(uploaded_file):
    try:
        ...
        return {"category":cat,"structure":struct,
                "color":{"dominant":dom,"brightness":round(brightness,1),"saturation":round(saturation,1),"mean_rgb":(round(mr,1),round(mg,1),round(mb,1)),"warm":warm},
                "texture":{"edge_intensity":round(edge,2),"contrast":round(contrast,2),"entropy":round(entropy,3),"complexity":"Alta" if entropy>5.5 else ("Média" if entropy>4 else "Baixa")},
                "shape":{"symmetry_score":round(sym,3),"symmetry_level":"Alta" if sym>0.78 else ("Média" if sym>0.52 else "Baixa")},
                "palette":palette,"confidence":round(conf,1),"size":orig,"skin_pct":round(skin_pct*100,1)}
...
def render_topnav():
    ...
    # Cosmetic bar (HTML only, no interaction)
    nav_spans="".join(f'<span style="font-size:.79rem;color:{"var(--blue-g)" if cur==k else "var(--t3)"};font-weight:{"600" if cur==k else "400"};padding:.38rem .65rem;border-radius:8px;background:{"rgba(37,99,235,.20)" if cur==k else "transparent"};border:{"1px solid rgba(37,99,235,.28)" if cur==k else "none"};">{sym} {lbl}</span>' for k,sym,lbl in NAV)
    av_html=f'<div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--navy),var(--blue));display:flex;align-items:center;justify-content:center;font-size:.78rem;font-weight:700;color:white;border:1.5px solid var(--bdr-l);overflow:hidden;flex-shrink:0;">{"<img src=\\'"+photo+"\\' style=\\'width:100%;height:100%;object-fit:cover;border-radius:50%\\'/>" if photo else in_}</div>'
    ...
    # Functional buttons in a tight row below (visually hidden behind nav)
...
def page_profile(target_email):
    ...
    email=st.session_state.current_user; is_me=email==target_email; is_fol=target_email in st.session_state.followed
    ...
    if not is_me:
        ...
        with c1:
            if st.button("✓ Seguindo" if is_fol else "+ Seguir",key="btn_pf"):
                if is_fol: st.session_state.followed.remove(target_email)
...
def share_modal(post_id, title):
    ...
    st.markdown(f"""<div class="card" style="padding:1.2rem;">
...
def page_feed():
    ...
    with col_side:
        ...
        shown=0
        for ue,ud in list(st.session_state.users.items()):
            if ue==email: continue
            uname=ud.get("name","?")
            if sq and sq.lower() not in uname.lower() and sq.lower() not in ud.get("area","").lower(): continue
            if shown>=6: break
            shown+=1
            uin=ini(uname); uphoto=ud.get("photo_b64")
            is_fol=ue in st.session_state.followed
            ca,cb=st.columns([3,1])
            with ca: st.markdown(f'<div style="display:flex;align-items:center;gap:7px;margin-bottom:2px;">{avh(uin,26,uphoto)}<div><div style="font-size:.8rem;font-weight:500;">{uname}</div><div style="font-size:.68rem;color:var(--t3);">{ud.get("area","")[:20]}</div></div></div>',unsafe_allow_html=True)
            with cb:
        ...
        for area,cnt in [("Quantum ML",42),("CRISPR 2026",38),("Neuroplasticidade",31),("LLMs Científicos",27)]:
            st.markdown(f'<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--bdr);font-size:.8rem;"><span style="color:var(--t2);">{area}</span><span style="color:var(--blue-g);font-weight:600;">{cnt}</span></div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)

def render_post(post, rec=False, show_profile_link=True):
    email=st.session_state.current_user; liked=email in post["liked_by"]; saved=email in post.get("saved_by",[])
    aemail=post.get("author_email",""); aphoto=get_photo(aemail); rec_b='<span class="b-rec" style="margin-left:6px;">Rec.</span>' if rec else ""
    st.markdown(f"""<div class="card">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:.9rem;">
        {avh(post['avatar'],40,aphoto)}
        <div style="flex:1;"><div style="font-weight:600;font-size:.91rem;">{post['author']}</div><div style="color:var(--t3);font-size:.73rem;">{post['area']} · {post['date']}</div></div>
    ...
    if st.session_state.get(f"sc_{post['id']}",False):
        for c in post["comments"]:
            st.markdown(f'<div style="background:var(--surface);border-radius:10px;padding:7px 12px;margin:3px 0;font-size:.82rem;border:1px solid var(--bdr);"><strong style="color:var(--blue-g);">{c["user"]}</strong>: {c["text"]}</div>',unsafe_allow_html=True)
        ...
        if st.button("Enviar",key=f"cs_{post['id']}"):
            if nc: post["comments"].append({"user":guser().get("name","Você"),"text":nc}); record(post["tags"],.8); save_db(); st.rerun()
...
def page_search():
    ...
    if q:
        ...
        with tab_web:
            ...
            else: st.markdown('<div style="color:var(--t3);text-align:center;padding:2rem;">Sem resultados online. Verifique conexão ou tente outros termos.</div>',unsafe_allow_html=True)
        with tab_all:
            if neb_res:
...
def render_search_post(post):
    ...
    st.markdown(f"""<div class="scard">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:.6rem;">
        ...
        <div style="flex:1;"><div style="font-size:.81rem;font-weight:600;">{post['author']}</div><div style="font-size:.7rem;color:var(--t3);">{post['area']} · {post['date']} · {badge(post['status'])}</div></div>
...
def render_web_article(a):
    ...
    with cc:
        if a.get("url"):
            st.markdown(f'<a href="{a["url"]}" target="_blank" style="color:var(--blue-g);font-size:.82rem;text-decoration:none;line-height:2.4;">Abrir ↗</a>',unsafe_allow_html=True)
...
def page_knowledge():
    ...
    nodes=st.session_state.knowledge_nodes; node_map={n["id"]:n for n in nodes}
    ex,ey,ez=[],[],[]
    for n in nodes:
        for conn in n.get("connections",[]):
            if conn in node_map:
                t=node_map[conn]; ex+=[n["x"],t["x"],None]; ey+=[n["y"],t["y"],None]; ez+=[n["z"],t["z"],None]
    fig=go.Figure()
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode="markers+text",
        marker=dict(size=[n.get("size",16) for n in nodes],color=[n.get("color","#2563eb") for n in nodes],opacity=.90,line=dict(color="rgba(200,220,255,.28)",width=1.5)),
        ...
        customdata=[len(n.get("connections",[])) for n in nodes],showlegend=False,
    ))
        ...
        with cd:
            ...
            if st.button("Adicionar",key="btn_kn"):
                if t1 and t2:
                    ...
                    if t1 not in ids: nodes.append({"id":t1,"x":random.uniform(.05,.92),"y":random.uniform(.05,.92),"z":random.uniform(.05,.92),"connections":[t2],"color":color,"size":16})
                    else:
                        for n in nodes:
                            if n["id"]==t1 and t2 not in n.get("connections",[]): n.setdefault("connections",[]).append(t2)
                    record([t1.lower(),t2.lower()],1.0); st.session_state.notifications.insert(0,f"Nova conexão: {t1} ↔ {t2}")
    ...
    with tab_map:
        for n in nodes:
            if n.get("connections"):
                ...
                st.markdown(f'<div style="display:flex;align-items:center;flex-wrap:wrap;gap:6px;background:var(--glass);border:1px solid var(--bdr);border-radius:12px;padding:9px 14px;margin-bottom:5px;"><span style="background:{n.get("color","#2563eb")}20;border:1px solid {n.get("color","#2563eb")}55;border-radius:8px;padding:3px 11px;font-size:.82rem;font-weight:600;color:{n.get("color","#2563eb")};white-space:nowrap;">{n["id"]}</span><span style="color:var(--t3);font-size:.78rem;">→</span>{conns}</div>',unsafe_allow_html=True)
    with tab_edit:
        ...
        if node_obj:
            ...
            with c_save:
                if st.button("Salvar edição",key="btn_edit_node"): node_obj.update({"x":nx,"y":ny,"z":nz,"size":ns,"color":nc}); st.success("Nó atualizado!"); st.rerun()
            with c_del:
                if st.button("Remover nó",key="btn_rm_node"): st.session_state.knowledge_nodes=[n for n in nodes if n["id"]!=sel]; st.rerun()
    st.markdown('</div>',unsafe_allow_html=True)
...
def page_folders():
    ...
    if not st.session_state.folders:
        ...
        for idx,(fname,fdata) in enumerate(list(st.session_state.folders.items())):
            ...
            with cols[idx%3]:
                st.markdown(f'<div class="card" style="text-align:center;"><div style="font-size:2.2rem;margin-bottom:8px;">📁</div><div style="font-family:\'Playfair Display\',serif;font-weight:700;font-size:.98rem;">{fname}</div><div style="color:var(--t3);font-size:.71rem;margin-top:3px;">{desc}</div><div style="color:var(--blue-g);font-size:.74rem;margin-top:5px;">{len(files)} arquivo(s)</div></div>',unsafe_allow_html=True)
                with st.expander("Abrir pasta"):
                    ...
                    for f in files: st.markdown(f'<div style="font-size:.81rem;padding:3px 0;color:var(--t2);border-bottom:1px solid var(--bdr);">📄 {f}</div>',unsafe_allow_html=True)
...
def page_analytics():
    ...
    pc=dict(plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",font=dict(color="#4a5e80",family="DM Sans"),margin=dict(l=10,r=10,t=40,b=10),xaxis=dict(showgrid=False,color="#4a5e80"),yaxis=dict(showgrid=True,gridcolor="rgba(37,99,235,.07)",color="#4a5e80"))
    with tab_perf:
        cols=st.columns(4)
        ...
        c1,c2=st.columns(2)
        with c1:
            fig=go.Figure(); fig.add_trace(go.Scatter(x=d["months"],y=d["views"],fill="tozeroy",fillcolor="rgba(37,99,235,.10)",line=dict(color="#3b7cf4",width=2.5),mode="lines+markers",marker=dict(size=5,color="#60a5fa")))
            fig.update_layout(title=dict(text="Visualizações mensais",font=dict(color="#e0e8ff",family="Playfair Display")),height=260,**pc); st.plotly_chart(fig,use_container_width=True)
        with c2:
            fig2=go.Figure(); fig2.add_trace(go.Bar(x=d["months"],y=d["citations"],marker=dict(color=d["citations"],colorscale=[[0,"#0a1628"],[1,"#06b6d4"]],line=dict(color="rgba(96,165,250,.22)",width=1))))
            fig2.update_layout(title=dict(text="Citações mensais",font=dict(color="#e0e8ff",family="Playfair Display")),height=260,**pc); st.plotly_chart(fig2,use_container_width=True)
        c3,c4=st.columns(2)
        with c3:
            my_posts=[p for p in st.session_state.feed_posts if p.get("author_email")==email]
            if my_posts:
                fig3=go.Figure()
                fig3.add_trace(go.Bar(name="Curtidas",x=[p["title"][:18]+"…" for p in my_posts],y=[p["likes"] for p in my_posts],marker_color="#2563eb"))
                fig3.add_trace(go.Bar(name="Comentários",x=[p["title"][:18]+"…" for p in my_posts],y=[len(p["comments"]) for p in my_posts],marker_color="#06b6d4"))
                fig3.update_layout(barmode="group",title=dict(text="Engajamento por pesquisa",font=dict(color="#e0e8ff",family="Playfair Display")),height=260,**pc,legend=dict(font=dict(color="#94a8d0"))); st.plotly_chart(fig3,use_container_width=True)
            else: st.markdown('<div class="card" style="text-align:center;padding:2.5rem;"><div style="color:var(--t3);">Publique pesquisas para ver engajamento</div></div>',unsafe_allow_html=True)
        with c4:
            ...
            fig4.update_layout(title=dict(text="Leitores por país",font=dict(color="#e0e8ff",family="Playfair Display")),height=260,plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",legend=dict(font=dict(color="#94a8d0")),margin=dict(l=10,r=10,t=40,b=10)); st.plotly_chart(fig4,use_container_width=True)
        fig_g=go.Figure(go.Indicator(mode="gauge+number",value=d.get("aceitacao",94),title={"text":"Taxa de Aceitação (%)","font":{"color":"#94a8d0","family":"DM Sans"}},number={"suffix":"%","font":{"color":"#60a5fa","size":34}},gauge={"axis":{"range":[0,100],"tickcolor":"#4a5e80"},"bar":{"color":"#2563eb"},"bgcolor":"rgba(10,17,40,.5)","bordercolor":"rgba(37,99,235,.3)","steps":[{"range":[0,50],"color":"rgba(239,68,68,.08)"},{"range":[50,80],"color":"rgba(245,158,11,.08)"},{"range":[80,100],"color":"rgba(16,185,129,.08)"}]}))
        fig_g.update_layout(height=200,paper_bgcolor="rgba(0,0,0,0)",font=dict(color="#94a8d0")); st.plotly_chart(fig_g,use_container_width=True)
        if d.get("notes"): st.markdown(f'<div class="abox"><div style="font-size:.78rem;color:var(--t3);margin-bottom:4px;">NOTAS</div><div style="color:var(--t2);font-size:.85rem;line-height:1.6;">{d["notes"]}</div></div>',unsafe_allow_html=True)
    with tab_pref:
        prefs=st.session_state.user_prefs.get(email,{})
        if prefs:
            ...
            for i,(tag,score) in enumerate(top):
                ...
                with (c1 if i%2==0 else c2):
                    st.markdown(f'<div style="display:flex;justify-content:space-between;font-size:.81rem;margin-bottom:3px;"><span style="color:var(--t2);">{tag}</span><span style="color:var(--blue-g);">{pct}%</span></div>',unsafe_allow_html=True); st.progress(pct/100)
        else: st.info("Interaja com pesquisas e buscas para construir seu perfil de interesses.")
        ...
        if area_tags:
            ...
            for p in area_posts[:3]: render_search_post(p)
    with tab_edit:
        ...
        with ce3: new_ac=st.number_input("Taxa de aceitação (%)",0,100,d.get("aceitacao",94),key="e_ac")
        st.markdown("**Visualizações mensais** (Mar → Fev):")
        new_views=[]; vc=st.columns(6)
        for i,(m,v) in enumerate(zip(d["months"][:6],d["views"][:6])):
            with vc[i]: new_views.append(st.number_input(m,0,value=v,key=f"ev_{i}"))
        vc2=st.columns(6)
        for i,(m,v) in enumerate(zip(d["months"][6:],d["views"][6:])):
            with vc2[i]: new_views.append(st.number_input(m,0,value=v,key=f"ev_{i+6}"))
    ...
    with col_res:
        if run and img_file:
            ...
            if rep:
                ...
                for col,(v,l) in zip([c1,c2,c3],[(rep['texture']['complexity'],"Complexidade"),(rep['shape']['symmetry_level'],"Simetria"),(rep['color']['dominant'],"Canal dominante")]):
                    with col: st.markdown(f'<div class="mbox"><div style="font-family:\'Playfair Display\',serif;font-size:1.25rem;font-weight:700;color:#60a5fa;">{v}</div><div class="mlbl">{l}</div></div>',unsafe_allow_html=True)
                r,g,b_v=rep['color']['mean_rgb']
                hex_col="#{:02x}{:02x}{:02x}".format(int(r),int(g),int(b_v))
                pal_html="".join(f'<div style="display:flex;flex-direction:column;align-items:center;gap:3px;"><div style="width:34px;height:34px;border-radius:8px;background:rgb{str(p)};border:1px solid rgba(255,255,255,.12);"></div><div style="font-size:.62rem;color:var(--t3);">#{"{:02x}{:02x}{:02x}".format(*p).upper()}</div></div>' for p in rep["palette"])
            ...
            st.markdown("""<div class="card" style="text-align:center;padding:4rem 2rem;">
              ...
              <div style="color:var(--t3);font-size:.79rem;line-height:1.7;">Suportado: PNG · JPG · WEBP · TIFF<br>Detecta: padrões biológicos · fluorescência · histologia<br>cristalografia · diagramas · imagens multiespectrais</div>
            </div>""",unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)
...
def page_chat():
    ...
    with col_c:
        ...
        for ue in st.session_state.chat_contacts:
            ...
            st.markdown(f'<div style="background:{bg};border:1px solid {bdr};border-radius:12px;padding:9px 11px;margin-bottom:5px;"><div style="display:flex;align-items:center;gap:8px;">{avh(uin,32,uphoto)}<div style="overflow:hidden;min-width:0;"><div style="font-size:.83rem;font-weight:600;display:flex;align-items:center;">{dot}{uname}</div><div style="font-size:.7rem;color:var(--t3);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{last}</div></div></div></div>',unsafe_allow_html=True)
        ...
        if st.button("Adicionar",key="btn_add_ct"):
            if new_contact in st.session_state.users and new_contact!=st.session_state.current_user:
    ...
    with col_m:
        if st.session_state.active_chat:
            ...
            # Header
            st.markdown(f'<div style="background:var(--glass);border:1px solid var(--bdr);border-radius:14px;padding:12px 16px;margin-bottom:1rem;display:flex;align-items:center;gap:12px;">{avh(cin,38,cphoto)}<div style="flex:1;"><div style="font-weight:700;font-size:.95rem;">{cname}</div><div style="font-size:.72rem;color:var(--ok);">🔒 Criptografia ativa · AES-256</div></div></div>',unsafe_allow_html=True)
            # Messages only
            for msg in msgs:
                is_me=msg["from"]=="me"
                cls="bme" if is_me else "bthem"
                st.markdown(f'<div class="{cls}">{msg["text"]}<div style="font-size:.66rem;color:var(--t3);margin-top:3px;text-align:{"right" if is_me else "left"};">{msg["time"]}</div></div>',unsafe_allow_html=True)
            # Input
...
def page_settings():
    ...
    with tab_p:
        ...
        st.markdown(f"""<div style="display:flex;align-items:center;gap:1.5rem;padding:1.2rem;background:var(--glass);border:1px solid var(--bdr);border-radius:var(--r-lg);margin-bottom:1.5rem;">
        ...
        if st.button("Salvar perfil",key="btn_sp"):
            ...
            if new_e!=email and new_e not in st.session_state.users:
                st.session_state.users[new_e]=st.session_state.users.pop(email); st.session_state.current_user=new_e; email=new_e
    ...
    with tab_s:
        ...
        st.markdown(f'<div style="background:var(--glass);border:1px solid var(--bdr);border-radius:14px;padding:14px;display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;"><div><div style="font-weight:600;font-size:.9rem;">2FA por e-mail</div><div style="font-size:.74rem;color:var(--t3);">{email}</div></div><span style="color:{"#10b981" if en else "#ef4444"};font-size:.83rem;font-weight:700;">{"✓ Ativo" if en else "✗ Inativo"}</span></div>',unsafe_allow_html=True)
    ...
    with tab_pr:
        ...
        items="".join(f'<div style="display:flex;align-items:center;gap:12px;background:rgba(16,185,129,.07);border:1px solid rgba(16,185,129,.18);border-radius:12px;padding:11px;"><div style="width:28px;height:28px;border-radius:8px;background:rgba(16,185,129,.14);display:flex;align-items:center;justify-content:center;color:#10b981;font-weight:700;font-size:.78rem;flex-shrink:0;">✓</div><div><div style="font-weight:600;color:#10b981;font-size:.86rem;">{n2}</div><div style="font-size:.73rem;color:var(--t3);">{d2}</div></div></div>' for n2,d2 in prots)
        ...
        c1,c2=st.columns(2)
        with c1: st.selectbox("Perfil",["Público","Só seguidores","Privado"],key="vp"); st.selectbox("Pesquisas",["Público","Só seguidores","Privado"],key="vr")
        with c2: st.selectbox("Estatísticas",["Público","Privado"],key="vs"); st.selectbox("Rede de conhecimento",["Público","Só seguidores","Privado"],key="vn")
        if st.button("Salvar privacidade",key="btn_priv"): st.success("✓ Configurações de privacidade salvas!")
    st.markdown('</div>',unsafe_allow_html=True)
