...
USER_GRADIENTS=["135deg,#FFD60A,#FF6D00","135deg,#00E676,#00BFA5","135deg,#FF1744,#D500F9","135deg,#2979FF,#00E5FF","135deg,#FF6D00,#FFD60A","135deg,#AA00FF,#2979FF","135deg,#00E5FF,#00E676","135deg,#FF4081,#FF1744"]
...
def get_db_manager():
    class DBManager:
        def __init__(self): self.load_initial_data()
        ...
        def save_db(self):
            try:
                ...
                with open(DB_FILE,"w",encoding="utf-8") as f:
                    json.dump({"users":st.session_state.users,"feed_posts":st.session_state.feed_posts,"folders":st.session_state.folders,"user_prefs":prefs_s,"saved_articles":st.session_state.saved_articles},f,ensure_ascii=False,indent=2)
            except Exception as e: st.error(f"Erro ao salvar: {e}")
    return DBManager()

db_manager=get_db_manager()
if "initialized" not in st.session_state: db_manager.load_initial_data()
...
def extract_text_from_csv_bytes(csv_bytes):
    try:
        ...
        for col in df.columns[:10]:
            ...
            else: s+=f"{col}: min={df[col].min():.2f}, max={df[col].max():.2f}\n"
        return s
    except: return ""
...
def extract_authors_from_text(text):
    authors=[]; seen=set()
    for pat in [r'(?:Autor(?:es)?|Author(?:s)?)[:\s]+([A-ZÀ-Ú][a-zà-ú]+(?:\s+[A-ZÀ-Ú][a-zà-ú]+){1,4})',r'(?:Por|By)[:\s]+([A-ZÀ-Ú][a-zà-ú]+(?:\s+[A-ZÀ-Ú][a-zà-ú]+){1,3})']:
...
def compute_topic_distribution(keywords):
    topic_map={"Saúde & Medicina":["saúde","medicina","hospital","doença","tratamento","clínico","health","medical","clinical","therapy","disease","cancer"],"Biologia & Genômica":["biologia","genômica","gene","dna","rna","proteína","célula","bacteria","crispr","genomics","biology","protein","cell"],"Neurociência":["neurociência","neural","cérebro","cognição","memória","sinapse","sono","brain","neuron","cognitive","memory","sleep"],"Computação & IA":["algoritmo","machine","learning","inteligência","neural","dados","computação","ia","modelo","algorithm","deep","quantum"],"Física & Astronomia":["física","quântica","partícula","energia","galáxia","astrofísica","cosmologia","physics","quantum","particle","galaxy","dark"],"Química & Materiais":["química","molécula","síntese","reação","composto","polímero","chemistry","molecule","synthesis","nanomaterial"],"Engenharia":["engenharia","sistema","robótica","automação","sensor","circuito","engineering","system","robotics"],"Ciências Sociais":["sociedade","cultura","educação","política","economia","social","psicologia","society","culture"],"Ecologia & Clima":["ecologia","clima","ambiente","biodiversidade","ecology","climate","environment","sustainability"],"Matemática & Estatística":["matemática","estatística","probabilidade","equação","mathematics","statistics","probability"]}
...
def compute_writing_quality(text,keywords,references):
    ...
    if len([k for k in keywords if len(k)>7])/max(len(keywords),1)>0.5: score+=10
    return min(100,score)

@st.cache_data(show_spinner=False)
def search_references_online(keywords,n=4):
    if not keywords: return []
    try:
        r=requests.get("https://api.semanticscholar.org/graph/v1/paper/search",params={"query":" ".join(keywords[:4]),"limit":n,"fields":"title,authors,year,abstract,venue,externalIds,citationCount"},timeout=7)
        if r.status_code==200:
            ...
            for p in r.json().get("data",[]):
                ...
                results.append({"title":p.get("title","?"),"authors":authors or "—","year":p.get("year","?"),"venue":p.get("venue","") or "—","abstract":(p.get("abstract","") or "")[:180],"url":url,"citations":p.get("citationCount",0),"doi":doi})
            return results
    except: pass
    return []
...
def analyze_document_intelligent(fname,fbytes,ftype,research_area=""):
    result={"file":fname,"type":ftype,"text_length":0,"keywords":[],"authors":[],"years":[],"references":[],"topics":{},"references_online":[],"relevance_score":0,"summary":"","strengths":[],"improvements":[],"writing_quality":0,"reading_time":0,"word_count":0,"concept_frequency":{},"sentence_complexity":0}
    text=""
    ...
    if text:
        ...
        wf=[w for w in re.findall(r'\b[a-záàâãéêíóôõúüçA-ZÁÀÂÃÉÊÍÓÔÕÚÜÇ]{4,}\b',text.lower()) if w not in STOPWORDS]
        result["concept_frequency"]=dict(Counter(wf).most_common(15))
        sentences=re.split(r'[.!?]+',text)
        result["sentence_complexity"]=round(sum(len(s.split()) for s in sentences)/max(len(sentences),1),1)
        if research_area:
            rel=sum(1 for w in research_area.lower().split() if any(w in kw for kw in result["keywords"]))
        ...
        if n_kw>15: result["strengths"].append(f"Vocabulário rico ({n_kw} termos)")
        ...
        result["summary"]=f"{ftype} · {words} palavras · ~{minutes}min · {', '.join(list(result['topics'].keys())[:3])} · {', '.join(result['keywords'][:5])}"
    else:
...
@st.cache_data(show_spinner=False)
def analyze_image_advanced(file_bytes):
    try:
        img=PILImage.open(io.BytesIO(file_bytes)).convert("RGB"); orig=img.size
        ...
        strengths={"Horizontal":float(np.abs(gy).mean()),"Vertical":float(np.abs(gx).mean()),"Diagonal A":float(np.abs(gx+gy).mean()),"Diagonal B":float(np.abs(gx-gy).mean())}
        ...
        fft_s=np.fft.fftshift(np.abs(np.fft.fft2(gray))); hf,wf2=fft_s.shape
        cm=np.zeros_like(fft_s,dtype=bool); cm[hf//2-22:hf//2+22,wf2//2-22:wf2//2+22]=True
        has_grid=float(np.percentile(fft_s[~cm],99))>float(np.mean(fft_s[~cm]))*14
        hist_a=np.histogram(gray,bins=64,range=(0,255))[0]; hn=hist_a/hist_a.sum(); hn=hn[hn>0]
        ...
        if skin_pct>0.15 and mr>140: cat,desc,kw,material,obj_type,context="Histopatologia H&E",f"Tecido orgânico ({skin_pct*100:.0f}%).","hematoxylin eosin histopathology","Tecido Biológico","Amostra Histopatológica","Microscopia óptica"
        elif has_grid and edge_intensity>18: cat,desc,kw,material,obj_type,context="Cristalografia","Padrão periódico.","X-ray diffraction crystallography","Material Cristalino","Rede Cristalina","Análise estrutura atômica"
        elif mg>165 and mr<125: cat,desc,kw,material,obj_type,context="Fluorescência GFP/FITC",f"Canal verde (G={mg:.0f}).","GFP fluorescence confocal","Proteínas Fluorescentes","Células Marcadas","Microscopia confocal"
        elif mb>165 and mr<110: cat,desc,kw,material,obj_type,context="Fluorescência DAPI",f"Canal azul (B={mb:.0f}).","DAPI nuclear staining DNA","DNA / Cromatina","Núcleos Celulares","Marcação nuclear"
        elif has_circular and edge_intensity>24: cat,desc,kw,material,obj_type,context="Microscopia Celular","Estruturas circulares.","cell organelle vesicle bacteria","Componentes Celulares","Células/Organelas","Microscopia campo claro"
        elif edge_intensity>40: cat,desc,kw,material,obj_type,context="Diagrama Científico","Bordas nítidas.","scientific visualization chart","Dados Estruturados","Gráfico/Diagrama","Visualização dados"
        elif sym>0.82: cat,desc,kw,material,obj_type,context="Estrutura Molecular",f"Alta simetria ({sym:.3f}).","molecular structure protein crystal","Moléculas","Estrutura Molecular","Visualização molecular 3D"
        else:
            temp="quente" if warm else("fria" if cool else "neutra")
            cat,desc,kw,material,obj_type,context="Imagem Científica Geral",f"Temperatura {temp}.","scientific image analysis","Variado","Imagem Científica","Análise genérica"
        conf=min(96,48+edge_intensity/2+entropy*2.8+sym*5+(8 if skin_pct>0.1 else 0)+(6 if has_grid else 0))
        return {"category":cat,"description":desc,"kw":kw,"material":material,"object_type":obj_type,"context":context,"confidence":round(conf,1),"lines":{"direction":line_dir,"intensity":round(edge_intensity,2),"strengths":strengths},"shapes":shapes,"symmetry":round(sym,3),"color":{"r":round(mr,1),"g":round(mg,1),"b":round(mb,1),"warm":warm,"cool":cool,"dom":dom_ch,"sat":round(sat*100,1)},"texture":{"entropy":round(entropy,3),"complexity":"Alta" if entropy>5.5 else("Média" if entropy>4 else "Baixa")},"palette":palette,"size":orig,"histograms":{"r":np.histogram(r.ravel(),bins=32,range=(0,255))[0].tolist(),"g":np.histogram(g.ravel(),bins=32,range=(0,255))[0].tolist(),"b":np.histogram(b_ch.ravel(),bins=32,range=(0,255))[0].tolist()},"brightness":round(float(gray.mean()),1),"sharpness":round(edge_intensity,2)}
...
def get_ftype(fname):
    ext=fname.split(".")[-1].lower() if "." in fname else ""; return EMAP.get(ext,"Arquivo")

@st.cache_data(show_spinner=False)
def search_ss(query,limit=6):
    results=[]
    try:
        r=requests.get("https://api.semanticscholar.org/graph/v1/paper/search",params={"query":query,"limit":limit,"fields":"title,authors,year,abstract,venue,externalIds,openAccessPdf,citationCount"},timeout=8)
        if r.status_code==200:
            for p in r.json().get("data",[]):
                ext=p.get("externalIds",{}) or {}; doi=ext.get("DOI",""); arxiv=ext.get("ArXiv","")
                ...
                alist=p.get("authors",[]) or []; authors=", ".join(a.get("name","") for a in alist[:3])
                if len(alist)>3: authors+=" et al."
                results.append({"title":p.get("title","Sem título"),"authors":authors or "—","year":p.get("year","?"),"source":p.get("venue","") or "Semantic Scholar","doi":doi or arxiv or "—","abstract":(p.get("abstract","") or "")[:260],"url":link,"citations":p.get("citationCount",0),"origin":"semantic"})
            return results
    except: pass
    return []

@st.cache_data(show_spinner=False)
def search_cr(query,limit=3):
    results=[]
    try:
        r=requests.get("https://api.crossref.org/works",params={"query":query,"rows":limit,"select":"title,author,issued,abstract,DOI,container-title,is-referenced-by-count","mailto":"nebula@example.com"},timeout=8)
        if r.status_code==200:
            for p in r.json().get("message",{}).get("items",[]):
                ...
                doi=p.get("DOI",""); abstract=re.sub(r'<[^>]+>','',p.get("abstract","") or "")[:260]
...
CHART_COLORS=["#FFD60A","#00E676","#FF1744","#2979FF","#D500F9","#00E5FF","#FF6D00","#FF4081","#69F0AE","#82B1FF"]

def pc():
    return dict(plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",font=dict(color="#60607A",family="Outfit",size=11),margin=dict(l=10,r=10,t=38,b=10),xaxis=dict(showgrid=False,color="#60607A",tickfont=dict(size=10)),yaxis=dict(showgrid=True,gridcolor="rgba(255,255,255,.05)",color="#60607A",tickfont=dict(size=10)))
...
:root{
  --bg0:#070712;--bg1:#0c0c1e;--bg2:#10102a;
  --yellow:#FFD60A;--yellow2:#FFE566;
  --green:#00E676;--green2:#69F0AE;
  --red:#FF1744;--red2:#FF6E6E;
  --blue:#2979FF;--blue2:#82B1FF;
  --purple:#D500F9;--cyan:#00E5FF;--orange:#FF6D00;--pink:#FF4081;
  --glass:rgba(255,255,255,0.055);
  --glass2:rgba(255,255,255,0.09);
  --glass3:rgba(255,255,255,0.14);
  --glassb:rgba(255,255,255,0.028);
  --border:rgba(255,255,255,0.09);
  --border2:rgba(255,255,255,0.16);
  --border3:rgba(255,255,255,0.28);
  --t0:#FFFFFF;--t1:#EEEEFF;--t2:#A0A0C8;--t3:#60607A;--t4:#35354A;
  --r6:6px;--r10:10px;--r14:14px;--r18:18px;--r24:24px;--r32:32px;
  --sh:0 8px 40px rgba(0,0,0,.7);--sh2:0 4px 20px rgba(0,0,0,.5);
}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
html,body,.stApp{background:var(--bg0)!important;color:var(--t1)!important;font-family:'Outfit',-apple-system,sans-serif!important}

.stApp::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;
  background:
    radial-gradient(ellipse 70% 55% at 5% 5%,rgba(255,214,10,.08) 0%,transparent 55%),
    radial-gradient(ellipse 55% 50% at 95% 95%,rgba(0,230,118,.07) 0%,transparent 50%),
    radial-gradient(ellipse 45% 40% at 50% 50%,rgba(41,121,255,.04) 0%,transparent 65%),
    radial-gradient(ellipse 40% 35% at 85% 15%,rgba(213,0,249,.06) 0%,transparent 55%),
    radial-gradient(ellipse 35% 30% at 15% 85%,rgba(255,23,68,.05) 0%,transparent 50%);}

.stApp::after{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;
  background-image:
    radial-gradient(1.5px 1.5px at 12% 18%,rgba(255,214,10,.55) 0%,transparent 100%),
    radial-gradient(1px 1px at 34% 55%,rgba(0,230,118,.45) 0%,transparent 100%),
    radial-gradient(1.5px 1.5px at 68% 12%,rgba(41,121,255,.50) 0%,transparent 100%),
    radial-gradient(1px 1px at 82% 72%,rgba(255,23,68,.45) 0%,transparent 100%),
    radial-gradient(1px 1px at 48% 88%,rgba(0,229,255,.40) 0%,transparent 100%),
    radial-gradient(1.5px 1.5px at 22% 78%,rgba(213,0,249,.45) 0%,transparent 100%),
    radial-gradient(1px 1px at 91% 38%,rgba(255,109,0,.42) 0%,transparent 100%),
    radial-gradient(1px 1px at 60% 42%,rgba(255,214,10,.28) 0%,transparent 100%),
    radial-gradient(1px 1px at 76% 55%,rgba(0,230,118,.30) 0%,transparent 100%),
    radial-gradient(1.2px 1.2px at 40% 25%,rgba(41,121,255,.35) 0%,transparent 100%);}

[data-testid="collapsedControl"]{display:none!important}
header[data-testid="stHeader"]{display:none!important}
#MainMenu,footer,.stDeployButton{display:none!important}
[data-testid="stToolbar"],[data-testid="stDecoration"]{display:none!important}
.block-container{padding-top:0!important;padding-bottom:4rem!important;max-width:1500px!important;position:relative;z-index:1;padding-left:.75rem!important;padding-right:.75rem!important}

/* ══ SIDEBAR ══ */
section[data-testid="stSidebar"]{
  background:linear-gradient(180deg,rgba(7,7,22,.97) 0%,rgba(12,12,30,.96) 100%)!important;
  backdrop-filter:blur(50px) saturate(220%)!important;-webkit-backdrop-filter:blur(50px) saturate(220%)!important;
  border-right:1px solid var(--border)!important;box-shadow:6px 0 60px rgba(0,0,0,.65)!important;
  width:224px!important;min-width:224px!important;max-width:224px!important;}
section[data-testid="stSidebar"]>div{padding:1.4rem .88rem 1rem!important}

.sb-logo .stButton>button{background:transparent!important;border:none!important;font-family:'Outfit',sans-serif!important;font-size:1.65rem!important;font-weight:900!important;letter-spacing:-.06em!important;background:linear-gradient(135deg,var(--yellow) 0%,var(--green) 45%,var(--blue) 100%)!important;-webkit-background-clip:text!important;-webkit-text-fill-color:transparent!important;background-clip:text!important;padding:.15rem .4rem!important;box-shadow:none!important;width:100%!important;height:auto!important;min-height:auto!important;text-align:center!important;}
.sb-logo .stButton>button:hover{transform:none!important;box-shadow:none!important}

.sb-nav .stButton>button{background:transparent!important;border:1px solid transparent!important;border-radius:var(--r10)!important;color:var(--t3)!important;font-family:'Outfit',sans-serif!important;font-weight:500!important;font-size:.84rem!important;padding:.50rem .82rem!important;width:100%!important;text-align:left!important;transition:all .15s!important;box-shadow:none!important;height:auto!important;min-height:auto!important;margin-bottom:.22rem!important;}
.sb-nav .stButton>button:hover{background:var(--glass2)!important;border-color:var(--border)!important;color:var(--t1)!important;transform:translateX(4px)!important;box-shadow:none!important}

.sb-nav-y.sb-active .stButton>button{background:linear-gradient(135deg,rgba(255,214,10,.15),rgba(255,109,0,.08))!important;border:1px solid rgba(255,214,10,.32)!important;color:var(--yellow)!important;font-weight:700!important;box-shadow:0 0 28px rgba(255,214,10,.12),inset 0 1px 0 rgba(255,255,255,.05)!important;}
.sb-nav-g.sb-active .stButton>button{background:linear-gradient(135deg,rgba(0,230,118,.15),rgba(0,229,255,.08))!important;border:1px solid rgba(0,230,118,.32)!important;color:var(--green)!important;font-weight:700!important;box-shadow:0 0 28px rgba(0,230,118,.12)!important;}
.sb-nav-b.sb-active .stButton>button{background:linear-gradient(135deg,rgba(41,121,255,.17),rgba(0,229,255,.08))!important;border:1px solid rgba(41,121,255,.34)!important;color:var(--blue2)!important;font-weight:700!important;box-shadow:0 0 28px rgba(41,121,255,.14)!important;}
.sb-nav-r.sb-active .stButton>button{background:linear-gradient(135deg,rgba(255,23,68,.15),rgba(255,109,0,.08))!important;border:1px solid rgba(255,23,68,.30)!important;color:var(--red2)!important;font-weight:700!important;box-shadow:0 0 28px rgba(255,23,68,.12)!important;}
.sb-nav-p.sb-active .stButton>button{background:linear-gradient(135deg,rgba(213,0,249,.15),rgba(41,121,255,.08))!important;border:1px solid rgba(213,0,249,.30)!important;color:#E040FB!important;font-weight:700!important;box-shadow:0 0 28px rgba(213,0,249,.12)!important;}
.sb-nav-c.sb-active .stButton>button{background:linear-gradient(135deg,rgba(0,229,255,.15),rgba(0,230,118,.08))!important;border:1px solid rgba(0,229,255,.32)!important;color:var(--cyan)!important;font-weight:700!important;box-shadow:0 0 28px rgba(0,229,255,.12)!important;}
.sb-nav-o.sb-active .stButton>button{background:linear-gradient(135deg,rgba(255,109,0,.15),rgba(255,214,10,.08))!important;border:1px solid rgba(255,109,0,.30)!important;color:#FFAB40!important;font-weight:700!important;box-shadow:0 0 28px rgba(255,109,0,.12)!important;}

.sb-av .stButton>button{width:38px!important;height:38px!important;min-height:38px!important;border-radius:50%!important;padding:0!important;font-family:'Outfit',sans-serif!important;font-weight:900!important;font-size:.74rem!important;color:white!important;border:1.5px solid rgba(255,255,255,.18)!important;box-shadow:0 2px 14px rgba(0,0,0,.5)!important;transition:all .18s!important;}
.sb-av .stButton>button:hover{transform:scale(1.12)!important;border-color:rgba(255,214,10,.5)!important;box-shadow:0 4px 22px rgba(255,214,10,.28)!important}

/* ══ TYPOGRAPHY ══ */
h1{font-family:'Outfit',sans-serif!important;font-size:1.5rem!important;font-weight:800!important;letter-spacing:-.03em;color:var(--t0)!important}
h2{font-family:'Outfit',sans-serif!important;font-size:1rem!important;font-weight:700!important;color:var(--t0)!important}
h3{font-family:'Outfit',sans-serif!important;font-size:.86rem!important;font-weight:600!important;color:var(--t1)!important}

/* ══ GLOBAL BUTTONS ══ */
.stButton>button{background:var(--glass)!important;backdrop-filter:blur(20px)!important;-webkit-backdrop-filter:blur(20px)!important;border:1px solid var(--border)!important;border-radius:var(--r10)!important;color:var(--t2)!important;font-family:'Outfit',sans-serif!important;font-weight:500!important;font-size:.78rem!important;padding:.40rem .80rem!important;transition:all .16s cubic-bezier(.4,0,.2,1)!important;box-shadow:var(--sh2)!important;}
.stButton>button:hover{background:var(--glass2)!important;border-color:var(--border2)!important;color:var(--t0)!important;transform:translateY(-2px)!important;box-shadow:0 8px 30px rgba(0,0,0,.6)!important}
.stButton>button:active{transform:scale(.96)!important}

.btn-y .stButton>button{background:linear-gradient(135deg,rgba(255,214,10,.18),rgba(255,109,0,.10))!important;border:1px solid rgba(255,214,10,.40)!important;color:var(--yellow)!important;font-weight:700!important;box-shadow:0 4px 22px rgba(255,214,10,.16),inset 0 1px 0 rgba(255,255,255,.07)!important;}
.btn-y .stButton>button:hover{background:linear-gradient(135deg,rgba(255,214,10,.30),rgba(255,109,0,.18))!important;border-color:rgba(255,214,10,.65)!important;box-shadow:0 8px 32px rgba(255,214,10,.30)!important}
.btn-g .stButton>button{background:linear-gradient(135deg,rgba(0,230,118,.17),rgba(0,229,255,.09))!important;border:1px solid rgba(0,230,118,.38)!important;color:var(--green)!important;font-weight:700!important;box-shadow:0 4px 22px rgba(0,230,118,.13)!important;}
.btn-g .stButton>button:hover{border-color:rgba(0,230,118,.62)!important;box-shadow:0 8px 32px rgba(0,230,118,.26)!important}
.btn-r .stButton>button{background:linear-gradient(135deg,rgba(255,23,68,.16),rgba(255,109,0,.08))!important;border:1px solid rgba(255,23,68,.35)!important;color:var(--red2)!important;font-weight:700!important;box-shadow:0 4px 22px rgba(255,23,68,.13)!important;}
.btn-r .stButton>button:hover{border-color:rgba(255,23,68,.58)!important;box-shadow:0 8px 32px rgba(255,23,68,.26)!important}
.btn-b .stButton>button{background:linear-gradient(135deg,rgba(41,121,255,.19),rgba(0,229,255,.09))!important;border:1px solid rgba(41,121,255,.40)!important;color:var(--blue2)!important;font-weight:700!important;box-shadow:0 4px 22px rgba(41,121,255,.15)!important;}
.btn-b .stButton>button:hover{border-color:rgba(41,121,255,.62)!important;box-shadow:0 8px 32px rgba(41,121,255,.28)!important}

.compose-prompt .stButton>button{background:var(--glassb)!important;border:1px solid var(--border)!important;border-radius:var(--r32)!important;color:var(--t3)!important;font-size:.83rem!important;font-weight:400!important;text-align:left!important;padding:.7rem 1.4rem!important;width:100%!important;box-shadow:none!important;}
.compose-prompt .stButton>button:hover{background:var(--glass)!important;border-color:var(--border2)!important;color:var(--t2)!important;transform:none!important}

/* ══ INPUTS ══ */
.stTextInput input,.stTextArea textarea{background:var(--glass)!important;backdrop-filter:blur(20px)!important;border:1px solid var(--border)!important;border-radius:var(--r10)!important;color:var(--t1)!important;font-family:'Outfit',sans-serif!important;font-size:.84rem!important;transition:border-color .15s,box-shadow .15s!important;}
.stTextInput input:focus,.stTextArea textarea:focus{border-color:rgba(255,214,10,.45)!important;box-shadow:0 0 0 3px rgba(255,214,10,.08)!important}
.stTextInput label,.stTextArea label,.stSelectbox label,.stFileUploader label,.stNumberInput label{color:var(--t3)!important;font-size:.62rem!important;letter-spacing:.09em!important;text-transform:uppercase!important;font-weight:600!important}

/* ══ AVATAR ══ */
.av{border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:'Outfit',sans-serif;font-weight:700;color:white;border:1.5px solid rgba(255,255,255,.12);flex-shrink:0;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.5)}

/* ══ CARDS ══ */
.card{background:var(--glass);backdrop-filter:blur(30px) saturate(180%);-webkit-backdrop-filter:blur(30px) saturate(180%);border:1px solid var(--border);border-radius:var(--r18);box-shadow:var(--sh),inset 0 1px 0 rgba(255,255,255,.05);position:relative;overflow:hidden}
.card::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.09),transparent);pointer-events:none}

.post{background:var(--glass);border:1px solid var(--border);border-radius:var(--r18);margin-bottom:.65rem;overflow:hidden;position:relative;box-shadow:var(--sh2),inset 0 1px 0 rgba(255,255,255,.04);animation:fadeUp .22s cubic-bezier(.34,1.1,.64,1) both;transition:border-color .16s,box-shadow .16s,transform .16s}
.post::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.07),transparent);pointer-events:none}
.post:hover{border-color:rgba(255,255,255,.16);box-shadow:0 8px 40px rgba(0,0,0,.7),0 0 0 1px rgba(255,255,255,.05)}

.compose-card{background:var(--glass);border:1px solid var(--border2);border-radius:var(--r18);padding:1.2rem 1.4rem;margin-bottom:.75rem;box-shadow:var(--sh),inset 0 1px 0 rgba(255,255,255,.07);animation:fadeUp .16s ease both}
.sc{background:var(--glass);border:1px solid var(--border);border-radius:var(--r18);padding:1rem 1.05rem;margin-bottom:.7rem}
.scard{background:var(--glass);border:1px solid var(--border);border-radius:var(--r14);padding:.85rem 1rem;margin-bottom:.5rem;transition:border-color .14s,transform .14s}
.scard:hover{border-color:var(--border2);transform:translateY(-1px)}
.abox{background:var(--glass);border:1px solid var(--border2);border-radius:var(--r14);padding:.95rem;margin-bottom:.7rem}
.pbox{background:rgba(0,230,118,.06);border:1px solid rgba(0,230,118,.18);border-radius:var(--r14);padding:.9rem;margin-bottom:.65rem}
.img-rc{background:rgba(0,230,118,.06);border:1px solid rgba(0,230,118,.16);border-radius:var(--r14);padding:.85rem;margin-bottom:.5rem}
.chart-glass{background:var(--glass);border:1px solid var(--border);border-radius:var(--r14);padding:.7rem;margin-bottom:.7rem}

.mbox{background:var(--glass);border:1px solid var(--border);border-radius:var(--r14);padding:.9rem;text-align:center}
.mval{font-family:'Outfit',sans-serif;font-size:1.65rem;font-weight:800;background:linear-gradient(135deg,var(--yellow),var(--orange));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.mval-g{font-family:'Outfit',sans-serif;font-size:1.65rem;font-weight:800;background:linear-gradient(135deg,var(--green),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.mlbl{font-size:.60rem;color:var(--t3);margin-top:3px;letter-spacing:.09em;text-transform:uppercase;font-weight:600}

.ai-warn{background:rgba(255,214,10,.07);border:1px solid rgba(255,214,10,.20);border-radius:var(--r10);padding:.65rem .95rem;margin:.45rem 0}
.str-ok{background:rgba(0,230,118,.07);border:1px solid rgba(0,230,118,.18);border-radius:9px;padding:.36rem .72rem;font-size:.74rem;color:var(--green2);margin-bottom:.28rem}
.str-imp{background:rgba(255,214,10,.07);border:1px solid rgba(255,214,10,.18);border-radius:9px;padding:.36rem .72rem;font-size:.74rem;color:var(--yellow2);margin-bottom:.28rem}
.ref-item{background:var(--glassb);border:1px solid var(--border);border-radius:var(--r10);padding:.6rem .85rem;font-size:.76rem;color:var(--t2);line-height:1.6}

/* ══ TABS ══ */
.stTabs [data-baseweb="tab-list"]{background:var(--glass)!important;border:1px solid var(--border)!important;border-radius:var(--r10)!important;padding:3px!important;gap:2px!important;}
.stTabs [data-baseweb="tab"]{background:transparent!important;color:var(--t3)!important;border-radius:8px!important;font-size:.76rem!important;font-family:'Outfit',sans-serif!important;font-weight:500!important;}
.stTabs [aria-selected="true"]{background:linear-gradient(135deg,rgba(255,214,10,.15),rgba(255,109,0,.08))!important;color:var(--yellow)!important;border:1px solid rgba(255,214,10,.25)!important;font-weight:700!important;}
.stTabs [data-baseweb="tab-panel"]{background:transparent!important;padding-top:.85rem!important;}

/* ══ BADGES / TAGS ══ */
.tag{display:inline-block;background:var(--glassb);border:1px solid var(--border);border-radius:20px;padding:2px 8px;font-size:.62rem;color:var(--t1);margin:2px;font-weight:500}
.badge-on{display:inline-block;background:rgba(255,214,10,.10);border:1px solid rgba(255,214,10,.22);border-radius:20px;padding:2px 8px;font-size:.62rem;font-weight:600;color:var(--yellow2)}
.badge-pub{display:inline-block;background:rgba(0,230,118,.10);border:1px solid rgba(0,230,118,.22);border-radius:20px;padding:2px 8px;font-size:.62rem;font-weight:600;color:var(--green2)}
.badge-done{display:inline-block;background:rgba(213,0,249,.10);border:1px solid rgba(213,0,249,.22);border-radius:20px;padding:2px 8px;font-size:.62rem;font-weight:600;color:var(--purple)}
.badge-rec{display:inline-block;background:rgba(255,109,0,.10);border:1px solid rgba(255,109,0,.22);border-radius:20px;padding:2px 8px;font-size:.62rem;font-weight:600;color:var(--orange)}

/* ══ INDICATORS / MISC ══ */
@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.75)}}
.dot-on{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--green);animation:pulse 2.5s infinite;margin-right:4px;vertical-align:middle}
...
.bme{background:linear-gradient(135deg,rgba(255,214,10,.15),rgba(255,109,0,.08));border:1px solid rgba(255,214,10,.25);border-radius:18px 18px 4px 18px;padding:.56rem .88rem;max-width:68%;margin-left:auto;margin-bottom:5px;font-size:.82rem;line-height:1.6}
.bthem{background:var(--glass);border:1px solid var(--border);border-radius:18px 18px 18px 4px;padding:.56rem .88rem;max-width:68%;margin-bottom:5px;font-size:.82rem;line-height:1.6}
.cmt{background:var(--glassb);border:1px solid var(--border);border-radius:var(--r10);padding:.52rem .85rem;margin-bottom:.28rem}

.prof-hero{background:var(--glass);backdrop-filter:blur(30px) saturate(180%);-webkit-backdrop-filter:blur(30px) saturate(180%);border:1px solid var(--border);border-radius:var(--r24);padding:1.6rem;display:flex;gap:1.3rem;align-items:flex-start;box-shadow:var(--sh);position:relative;overflow:hidden;margin-bottom:1.1rem}
.prof-photo{width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,var(--yellow),var(--orange));border:2px solid rgba(255,255,255,.15);flex-shrink:0;overflow:hidden;display:flex;align-items:center;justify-content:center;font-size:1.7rem;font-weight:700;color:white;box-shadow:0 4px 18px rgba(0,0,0,.5)}

.person-row{display:flex;align-items:center;gap:8px;padding:.42rem .48rem;border-radius:var(--r10);border:1px solid transparent;transition:all .14s;margin-bottom:2px}
.person-row:hover{background:var(--glassb);border-color:var(--border)}

.dtxt{display:flex;align-items:center;gap:.7rem;margin:.8rem 0;font-size:.60rem;color:var(--t3);letter-spacing:.09em;text-transform:uppercase;font-weight:600}
.dtxt::before,.dtxt::after{content:'';flex:1;height:1px;background:var(--border)}

hr{border:none;border-top:1px solid var(--border)!important;margin:.85rem 0}
label{color:var(--t2)!important}
.stCheckbox label,.stRadio label{color:var(--t1)!important}
.stAlert{background:var(--glass)!important;border:1px solid var(--border)!important;border-radius:var(--r14)!important}
.stSelectbox [data-baseweb="select"]{background:var(--glass)!important;border:1px solid var(--border)!important;border-radius:var(--r10)!important}
.stFileUploader section{background:var(--glass)!important;border:1.5px dashed var(--border2)!important;border-radius:var(--r14)!important}
.stExpander{background:var(--glass);border:1px solid var(--border);border-radius:var(--r14)}
.stRadio > div{display:flex!important;gap:4px!important;flex-wrap:wrap!important}
.stRadio > div > label{background:var(--glass)!important;border:1px solid var(--border)!important;border-radius:50px!important;padding:.28rem .78rem!important;font-size:.74rem!important;cursor:pointer!important;color:var(--t2)!important}
.stRadio > div > label:hover{border-color:var(--border2)!important;color:var(--t1)!important}
input[type="number"]{background:var(--glass)!important;border:1px solid var(--border)!important;border-radius:var(--r10)!important;color:var(--t1)!important}
...
def avh(initials,sz=40,grad=None):
    fs=max(sz//3,9); bg=grad or "linear-gradient(135deg,var(--yellow),var(--orange))"
    ...
    with col:
        ...
        <div style="text-align:center;margin-bottom:2.5rem">
          <div style="font-family:'Outfit',sans-serif;font-size:4rem;font-weight:900;
            background:linear-gradient(135deg,var(--yellow) 15%,var(--green) 55%,var(--blue) 100%);
        ...
        with t_in:
            with st.form("login_form"):
                ...
                st.markdown('<div class="btn-y">',unsafe_allow_html=True)
...
def page_verify_email():
    ...
    with col:
        ...
        <div class="card" style="padding:2rem;text-align:center">
          ...
          <p style="color:var(--t2);font-size:.82rem">Código para <strong style="color:var(--yellow)">{pv['email']}</strong></p>
          <div style="background:var(--glass2);border:1px solid var(--border);border-radius:12px;padding:1rem;margin:1rem 0">
            ...
            <div style="font-family:'Outfit',sans-serif;font-size:2.6rem;font-weight:900;letter-spacing:.28em;color:var(--yellow)">{pv['code']}</div>
          </div>
        </div>""",unsafe_allow_html=True)
        with st.form("verify_form"):
            ...
            if sub:
                if typed.strip()==pv["code"]:
                    ...
                    db_manager.save_db(); st.session_state.pending_verify=None
...
def page_2fa():
    ...
    with col:
        ...
        <div class="card" style="padding:2rem;text-align:center">
          ...
          <div style="background:var(--glass2);border:1px solid var(--border);border-radius:12px;padding:.9rem;margin:1rem 0">
            ...
            <div style="font-family:'Outfit',sans-serif;font-size:2.6rem;font-weight:900;letter-spacing:.26em;color:var(--yellow)">{p2['code']}</div>
          </div>
        </div>""",unsafe_allow_html=True)
...
NAV_ITEMS=[
    ("feed","🏠 Feed","y"),
    ("search","🔍 Busca","g"),
    ("knowledge","🕸 Conexões","b"),
    ("folders","📁 Pastas","r"),
    ("analytics","📊 Análises","p"),
    ("img_search","🔬 Imagem","c"),
    ("chat","💬 Chat","o"),
]

def render_sidebar_nav():
    u=guser(); name=u.get("name","?"); in_=ini(name); cur=st.session_state.page
    ...
    with st.sidebar:
        ...
        if st.button("🔬 Nebula",key="sidebar_logo"):
            st.session_state.profile_view=None; st.session_state.page="feed"; st.rerun()
        st.markdown('</div>',unsafe_allow_html=True)

        st.markdown('<div style="margin-bottom:1.5rem">',unsafe_allow_html=True)
        for key,label,color_class in NAV_ITEMS:
            active=(cur==key)
            cls=f"sb-nav-{color_class} sb-active" if active else "sb-nav"
            st.markdown(f'<div class="{cls}">',unsafe_allow_html=True)
            if st.button(label,key=f"snav_{key}",use_container_width=True):
        ...
        st.markdown('<div style="display:flex;align-items:center;gap:10px;margin-bottom:1rem">',unsafe_allow_html=True)
        nb=""
        if notif: nb=f'<div style="position:absolute;top:-1px;right:-1px;background:var(--red);color:white;width:13px;height:13px;border-radius:50%;font-size:.46rem;display:flex;align-items:center;justify-content:center;font-weight:700;z-index:10;pointer-events:none">{notif}</div>'
        st.markdown(f'<div style="position:relative;display:inline-block">{nb}</div>',unsafe_allow_html=True)
        st.markdown('<div class="sb-av">',unsafe_allow_html=True)
        if st.button(in_,key="snav_me",help="Ver Perfil"):
            st.session_state.profile_view=email; st.rerun()
        st.markdown('</div>',unsafe_allow_html=True)
        st.markdown(f'<div style="font-family:Outfit,sans-serif;font-weight:700;font-size:.88rem;color:var(--t1)">{name}</div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

def page_profile(target_email):
    tu=st.session_state.users.get(target_email,{})
    if not tu: st.error("Perfil não encontrado."); return
    tname=tu.get("name","?"); tin=ini(tname)
    ...
    photo_html=f'<span style="font-size:1.85rem;font-family:Outfit,sans-serif">{tin}</span>'
    v_badge=' <span style="font-size:.68rem;color:var(--green);margin-left:5px">✓</span>' if tu.get("verified") else ""
    st.markdown(f"""
    <div class="prof-hero">
      ...
      <div style="flex:1;z-index:1">
        ...
        <div style="color:var(--yellow);font-size:.82rem;margin-bottom:.4rem;font-weight:600">{tu.get("area","")}</div>
    ...
    if not is_me:
        ...
        with c1:
            btn_cls="btn-g" if is_fol else "btn-y"
        ...
        tab_my_data,tab_activity,tab_security=st.tabs(["  ✏️ Meus Dados  ",f"  ✨ Atividade ({len(user_posts)+len(liked_posts)+len(st.session_state.saved_articles)})  ","  🔐 Segurança  ",])
        with tab_my_data:
            ...
            with c_save:
                ...
                if st.button("💾 Salvar Dados",key="btn_sp",use_container_width=True):
                    ...
                    db_manager.save_db(); record(area_to_tags(new_a),1.5); st.success("✓ Perfil salvo!"); st.rerun()
                st.markdown('</div>',unsafe_allow_html=True)
            with c_out:
            ...
            if st.session_state.saved_articles:
                for idx,a in enumerate(st.session_state.saved_articles):
                    ...
                    if st.button("🗑 Remover",key=f"rms_{uid}"):
                        ...
                        db_manager.save_db(); st.toast("Removido!"); st.rerun()
                    st.markdown('</div>',unsafe_allow_html=True)
            else:
        ...
        with tab_security:
            ...
            with st.form("change_pw_form"):
                ...
                if st.form_submit_button("🔑 Alterar Senha"):
                    ...
                    else: st.session_state.users[email]["password"]=hp(np_); db_manager.save_db(); st.success("✓ Alterada!")
                st.markdown('</div>',unsafe_allow_html=True)
            st.markdown("<hr>",unsafe_allow_html=True)
            en=tu.get("2fa_enabled",False)
            st.markdown(f'<div class="card" style="padding:.9rem 1.2rem;display:flex;align-items:center;justify-content:space-between;margin-bottom:.9rem"><div style="flex:1"><div style="font-weight:700;font-size:.87rem">🔐 2FA — {"<span style=\'color:var(--green)\'>Ativo</span>" if en else "<span style=\'color:var(--red)\'>Inativo</span>"}</div><div style="font-size:.69rem;color:var(--t3)">{email}</div></div></div>',unsafe_allow_html=True)
            btn_2fa_cls="btn-r" if en else "btn-g"
            st.markdown(f'<div class="{btn_2fa_cls}">',unsafe_allow_html=True)
            if st.button("✕ Desativar 2FA" if en else "✓ Ativar 2FA",key="btn_2fa"):
                st.session_state.users[email]["2fa_enabled"]=not en; db_manager.save_db(); st.rerun()
            st.markdown('</div>',unsafe_allow_html=True)
            st.markdown("<hr>",unsafe_allow_html=True)
            ...
            for n2,d2 in prots:
                st.markdown(f'<div style="display:flex;align-items:center;gap:10px;background:rgba(0,230,118,.05);border:1px solid rgba(0,230,118,.13);border-radius:10px;padding:10px;margin-bottom:7px"><div style="width:26px;height:26px;border-radius:7px;background:rgba(0,230,118,.10);display:flex;align-items:center;justify-content:center;color:var(--green);font-size:.75rem;flex-shrink:0">✓</div><div><div style="font-weight:600;color:var(--green);font-size:.80rem">{n2}</div><div style="font-size:.68rem;color:var(--t3)">{d2}</div></div></div>',unsafe_allow_html=True)
...
def render_post(post,ctx="feed",show_author=True,compact=False):
    ...
    aemail=post.get("author_email",""); ain=post.get("avatar","??"); aname=post.get("author","?"); aarea=post.get("area","")
    dt=time_ago(post.get("date","")); views=post.get("views",200)
    abstract=post.get("abstract","")
    if compact and len(abstract)>200: abstract=abstract[:200]+"…"
    g=ugrad(aemail)
    if show_author:
        av_html=f'<div class="av" style="width:40px;height:40px;background:{g};font-size:12px">{ain}</div>'
        v_mark=' <span style="font-size:.58rem;color:var(--green)">✓</span>' if st.session_state.users.get(aemail,{}).get("verified") else ""
        header=(f'<div style="padding:.85rem 1.15rem .6rem;display:flex;align-items:center;gap:9px;border-bottom:1px solid var(--border)">'
                f'{av_html}'
    ...
    if st.session_state.get(f"shr_{ctx}_{pid}",False):
        ...
        st.markdown(
            ...
            f'<a href="https://twitter.com/intent/tweet?text={te}" target="_blank" style="text-decoration:none"><div style="background:rgba(41,121,255,.08);border:1px solid rgba(41,121,255,.16);border-radius:8px;padding:.32rem .65rem;font-size:.70rem;color:var(--blue)">𝕏 Twitter</div></a>'
            f'<a href="https://linkedin.com/sharing/share-offsite/?url={url}" target="_blank" style="text-decoration:none"><div style="background:rgba(41,121,255,.08);border:1px solid rgba(41,121,255,.16);border-radius:8px;padding:.32rem .65rem;font-size:.70rem;color:var(--blue)">in LinkedIn</div></a>'
            f'<a href="https://wa.me/?text={te}%20{url}" target="_blank" style="text-decoration:none"><div style="background:rgba(0,230,118,.07);border:1px solid rgba(0,230,118,.14);border-radius:8px;padding:.32rem .65rem;font-size:.70rem;color:var(--green)">📱 WhatsApp</div></a>'
            f'</div></div>',unsafe_allow_html=True)
    if st.session_state.get(f"cmt_{ctx}_{pid}",False):
        ...
        for c in comments:
            ...
            st.markdown(f'<div class="cmt"><div style="display:flex;align-items:center;gap:7px;margin-bottom:.22rem">{avh(c_in,26,c_grad)}<span style="font-size:.74rem;font-weight:600;color:var(--yellow)">{c["user"]}</span></div><div style="font-size:.78rem;color:var(--t2);line-height:1.55;padding-left:33px">{c["text"]}</div></div>',unsafe_allow_html=True)
        ...
        if st.button("→ Enviar",key=f"cs_{ctx}_{pid}"):
            if nc_txt:
                ...
                record(post.get("tags",[]),.8); db_manager.save_db(); st.rerun()
        st.markdown('</div>',unsafe_allow_html=True)

def page_feed():
    st.markdown('<div class="pw">',unsafe_allow_html=True)
    email=st.session_state.current_user; u=guser()
    uname=u.get("name","?"); uin=ini(uname)
    ...
    with col_main:
        ...
        if compose_open:
            ...
            with cpub:
                ...
                if st.button("🚀 Publicar",key="btn_pub",use_container_width=True):
                    if not np_t or not np_ab: st.warning("Título e resumo obrigatórios.")
                        ...
                        new_p={"id":len(st.session_state.feed_posts)+200+hash(np_t)%99,"author":uname,"author_email":email,"avatar":uin,"area":u.get("area",""),"title":np_t,"abstract":np_ab,"tags":tags,"likes":0,"comments":[],"status":np_st,"date":datetime.now().strftime("%Y-%m-%d"),"liked_by":[],"saved_by":[],"connections":tags[:3],"views":1}
                        st.session_state.feed_posts.insert(0,new_p)
                        record(tags,2.0); db_manager.save_db(); st.session_state.compose_open=False; st.rerun()
                st.markdown('</div>',unsafe_allow_html=True)
            with ccan:
            ...
            av_c2=f'<div class="av" style="width:38px;height:38px;font-size:12px;flex-shrink:0;background:{g}">{uin}</div>'
    ...
    with col_side:
        ...
        for ue,ud in list(users.items()):
            ...
            uin_r=ini(rname); rg=ugrad(ue)
            online=is_online(ue)
            dot='<span class="dot-on"></span>' if online else '<span class="dot-off"></span>'
            av_r=avh(uin_r,32,rg)
            st.markdown(f'<div class="person-row">{av_r}<div style="flex:1;min-width:0"><div><div style="font-size:.78rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{dot}{rname}</div><div style="font-size:.62rem;color:var(--t3)">{ud.get("area","")[:22]}</div></div></div></div>',unsafe_allow_html=True)
        ...
        for i,(topic,cnt) in enumerate(trending):
            ...
            st.markdown(f'<div style="padding:.38rem .32rem;border-radius:9px;margin-bottom:2px"><div><div><div style="font-size:.59rem;color:var(--t3);margin-bottom:1px">#{i+1}</div><div style="font-size:.78rem;font-weight:600;color:var(--t0)">{topic}</div><div style="font-size:.59rem;color:var(--t3)">{cnt} pesquisas</div></div></div></div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
        if st.session_state.notifications:
            ...
            for notif in st.session_state.notifications[:3]:
                st.markdown(f'<div style="font-size:.71rem;color:var(--t2);padding:.32rem 0;border-bottom:1px solid var(--border)">· {notif}</div>',unsafe_allow_html=True)
            st.markdown('</div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)

def render_web_article(a,idx=0,ctx="web"):
    src_color="var(--green)" if a.get("origin")=="semantic" else "var(--yellow)"
    ...
    with cc:
        if a.get("url"):
            st.markdown(f'<a href="{a["url"]}" target="_blank" style="color:var(--yellow);font-size:.78rem;text-decoration:none;line-height:2.4;display:block">↗ Abrir artigo</a>',unsafe_allow_html=True)
...
def page_search():
    ...
    if st.session_state.get("search_results") and st.session_state.get("last_sq"):
        ...
        with t_all:
            if neb:
                st.markdown('<div style="font-size:.61rem;color:var(--yellow);font-weight:700;margin-bottom:.45rem;letter-spacing:.09em;text-transform:uppercase">Na Nebula</div>',unsafe_allow_html=True)
                for p in neb: render_post(p,ctx="srch_all",compact=True)
            if web:
                if neb: st.markdown('<hr>',unsafe_allow_html=True)
                st.markdown('<div style="font-size:.61rem;color:var(--green);font-weight:700;margin-bottom:.45rem;letter-spacing:.09em;text-transform:uppercase">Bases Acadêmicas</div>',unsafe_allow_html=True)
...
def page_knowledge():
    ...
    for e1,e2,common,strength in edges:
        ...
        fig.add_trace(go.Scatter3d(x=[p1["x"],p2["x"],None],y=[p1["y"],p2["y"],None],z=[p1["z"],p2["z"],None],mode="lines",line=dict(color=f"rgba(255,255,255,{alpha:.2f})",width=min(4,1+strength)),hoverinfo="none",showlegend=False))
    ncolors=["#FFD60A" if ue==email else ("#00E676" if ue in st.session_state.followed else "#2979FF") for ue in rlist]
    ...
    with tab_map:
        for e1,e2,common,strength in sorted(edges,key=lambda x:-x[3])[:20]:
            ...
            st.markdown(f'<div class="scard"><div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap"><span style="font-size:.80rem;font-weight:700;font-family:Outfit,sans-serif;color:var(--yellow)">{n1.get("name","?")}</span><span style="color:var(--t3)">↔</span><span style="font-size:.80rem;font-weight:700;font-family:Outfit,sans-serif;color:var(--yellow)">{n2.get("name","?")}</span><div style="flex:1">{ts}</div><span style="font-size:.65rem;color:var(--green);font-weight:700">{strength}pt</span></div></div>',unsafe_allow_html=True)
    with tab_mine:
        ...
        for e1,e2,common,strength in sorted(my_conn,key=lambda x:-x[3]):
            ...
            st.markdown(f'<div class="scard"><div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap">{avh(ini(od.get("name","?")),36,og)}<div style="flex:1"><div><div style="font-weight:700;font-size:.84rem;font-family:Outfit,sans-serif">{od.get("name","?")}</div><div style="font-size:.68rem;color:var(--t3)">{od.get("area","")}</div></div></div>{tags_html(common[:3])}</div></div>',unsafe_allow_html=True)
    ...
    with tab_all:
        ...
        for ue,ud in users.items():
            ...
            st.markdown(f'<div class="scard"><div style="display:flex;align-items:center;gap:9px">{avh(ini(rn),36,rg)}<div style="flex:1"><div><div style="font-size:.84rem;font-weight:700;font-family:Outfit,sans-serif">{rn}</div><div style="font-size:.68rem;color:var(--t3)">{uarea}</div></div></div></div></div>',unsafe_allow_html=True)
...
def render_document_analysis(fname,analysis,research_area=""):
    ...
    rel_color="var(--green)" if rel>=70 else ("var(--yellow)" if rel>=45 else "var(--red)")
    wq_color="var(--green)" if wq>=70 else ("var(--yellow)" if wq>=45 else "var(--red)")
    ...
    <div class="abox">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:9px;margin-bottom:.55rem">
        <div style="flex:1">
          ...
          <div style="color:var(--t2);font-size:.78rem;line-height:1.62">{analysis.get("summary","")}</div>
        </div>
      ...
      <div style="display:flex;gap:1.4rem;flex-wrap:wrap;margin-top:.45rem;font-size:.65rem;color:var(--t3)">
    ...
    with tab_kw:
        if kws:
            ...
            fig=go.Figure(go.Bar(x=weights[:20],y=kws[:20],orientation='h',marker=dict(color=weights[:20],colorscale=[[0,"var(--bg2)"],[.4,"var(--yellow)"],[.7,"var(--orange)"],[1,"var(--green)"]],line=dict(color="var(--bg2)",width=1)),text=kws[:20],textposition='inside',textfont=dict(color='white',size=9)))
            layout={**pc(),'height':max(310,len(kws[:20])*17),'yaxis':dict(showticklabels=False),'title':dict(text="TF-IDF Keywords",font=dict(color="var(--t1)",family="Outfit",size=12))}
    ...
    with tab_topics:
        if topics:
            fig_pie=go.Figure(go.Pie(labels=list(topics.keys()),values=list(topics.values()),hole=0.50,marker=dict(colors=CHART_COLORS[:len(topics)],line=dict(color=["var(--bg2)"]*15,width=2)),textfont=dict(color="white",size=9),hoverinfo="label+percent"))
            fig_pie.update_layout(height=290,title=dict(text="Distribuição Temática",font=dict(color="var(--t1)",family="Outfit",size=12)),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",legend=dict(font=dict(color="var(--t3)",size=9)),margin=dict(l=0,r=0,t=38,b=0))
            st.markdown('<div class="chart-glass">',unsafe_allow_html=True)
            st.plotly_chart(fig_pie,use_container_width=True)
            st.markdown('</div>',unsafe_allow_html=True)
            for i,(topic,score) in enumerate(list(topics.items())[:8]):
                st.markdown(f'<div style="display:flex;align-items:center;gap:7px;margin-bottom:.38rem"><span>{topic}</span><span style="font-size:.68rem;color:var(--t3);width:26px;text-align:right">{score}</span></div>',unsafe_allow_html=True)
        else: st.info("Análise temática não disponível.")
    with tab_authors:
        if authors:
            for author in authors:
                g_a=ugrad(author)
                st.markdown(f'<div style="display:flex;align-items:center;gap:7px;padding:.38rem 0;border-bottom:1px solid var(--border)"><div style="width:26px;height:26px;border-radius:50%;background:{g_a};display:flex;align-items:center;justify-content:center;font-size:.63rem;font-weight:700;color:white;flex-shrink:0">{ini(author)}</div><span style="font-size:.80rem;color:var(--t1)">{author}</span></div>',unsafe_allow_html=True)
        ...
        if years:
            ...
            fig_y=go.Figure(go.Bar(x=yl,y=yv,marker=dict(color=yv,colorscale=[[0,"var(--bg2)"],[.5,"var(--yellow)"],[1,"var(--orange)"]]),text=yv,textposition="outside",textfont=dict(color="var(--t3)",size=9)))
            fig_y.update_layout(height=185,title=dict(text="Anos Citados",font=dict(color="var(--t1)",family="Outfit",size=11)),**pc())
            st.markdown('<div class="chart-glass">',unsafe_allow_html=True)
    ...
    with tab_refs:
        ...
        if refs_online:
            ...
            for i,ref in enumerate(refs_online[:5]):
                url_html=f'<a href="{ref["url"]}" target="_blank" style="color:var(--yellow);text-decoration:none;font-size:.70rem">↗ Abrir</a>' if ref.get("url") else ""
    ...
    with tab_improve:
        wq_color2="var(--green)" if wq>=70 else ("var(--yellow)" if wq>=45 else "var(--red)")
        st.markdown(f'<div class="pbox"><div style="font-family:Outfit,sans-serif;font-weight:700;font-size:.83rem;margin-bottom:.65rem;color:var(--green)">📊 Qualidade de Escrita</div><div style="font-size:.74rem;color:var(--t2);margin-bottom:.7rem">Qualidade: <strong style="color:{wq_color2}">{wq}% — {wq_label}</strong> · Complexidade: <strong>{sc:.1f} pal/frase</strong></div></div>',unsafe_allow_html=True)
        if strengths_a:
...
def page_folders():
    ...
    st.markdown('<div class="btn-y" style="display:inline-block">',unsafe_allow_html=True)
    if st.button("📁 Criar pasta",key="btn_nf"):
        if nf_name.strip():
            if nf_name not in st.session_state.folders:
                ...
                db_manager.save_db(); st.success(f"Pasta '{nf_name}' criada!"); st.rerun()
    ...
    for fname,fdata in list(st.session_state.folders.items()):
        ...
        with st.expander(f"📁 {fname} — {len(files)} arquivo(s)"):
            ...
            if up:
                ...
                fdata["files"]=files; db_manager.save_db(); st.success(f"{len(up)} arquivo(s) adicionado(s)!")
            if files:
                for f in files:
                    ...
                    st.markdown(f'<div style="display:flex;align-items:center;gap:7px;padding:.40rem 0;border-bottom:1px solid var(--border)"><span>{icon}</span><span style="font-size:.77rem;color:var(--t2);flex:1">{f}</span>{ab}</div>',unsafe_allow_html=True)
            else:
            ...
            with ca_btn:
                ...
                if st.button("🔬 Analisar",key=f"analyze_{fname}",use_container_width=True):
                    if files:
                        ...
                        db_manager.save_db(); pb.empty(); st.success("✓ Análise completa!"); st.rerun()
            ...
            if st.button("💾 Salvar nota",key=f"sn_{fname}"):
                fdata["notes"]=note; db_manager.save_db(); st.success("✓ Nota salva!")
            st.markdown('</div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)
...
def page_analytics():
    ...
    with tab_f:
        ...
        if not folders:
            ...
            if all_topics:
                ...
                layout_t={**pc(),'height':270,'yaxis':dict(showgrid=False,color="var(--t3)",tickfont=dict(size=9)),'title':dict(text="Temas",font=dict(color="var(--t1)",family="Outfit",size=12))}
    ...
    with tab_p:
        ...
        if not my_posts:
            ...
            fig_eng.update_layout(barmode="group",title=dict(text="Engajamento",font=dict(color="var(--t1)",family="Outfit",size=12)),height=250,**pc(),legend=dict(font=dict(color="var(--t3)")))
            st.markdown('<div class="chart-glass">',unsafe_allow_html=True)
    ...
    with tab_i:
        ...
        with c3: st.markdown(f'<div class="mbox"><div class="mval-g">{len(st.session_state.saved_articles)}</div><div class="mlbl">Artigos Salvos</div></div>',unsafe_allow_html=True)
        st.markdown("<hr>",unsafe_allow_html=True)
        new_h=st.number_input("Índice H",0,200,d.get("h_index",4),key="e_h_ana")
        new_fi=st.number_input("Fator de impacto",0.0,100.0,float(d.get("fator_impacto",3.8)),step=0.1,key="e_fi_ana")
        new_notes=st.text_area("Notas",value=d.get("notes",""),key="e_notes",height=78)
        st.markdown('<div class="btn-y">',unsafe_allow_html=True)
        if st.button("💾 Salvar métricas",key="btn_save_m_ana"):
            d.update({"h_index":new_h,"fator_impacto":new_fi,"notes":new_notes}); db_manager.save_db(); st.success("✓ Salvo!")
        st.markdown('</div>',unsafe_allow_html=True)
    with tab_pr:
        prefs=st.session_state.user_prefs.get(email,{})
        if prefs:
            ...
            if len(cats)>=3:
                fig_r=go.Figure(go.Scatterpolar(r=vals+[vals[0]],theta=cats+[cats[0]],fill='toself',line=dict(color="#FFD60A"),fillcolor="rgba(255,214,10,.13)"))
            ...
            for i,(tag,score) in enumerate(top):
                with (c1 if i%2==0 else c2):
                    st.markdown(f'<div style="display:flex;justify-content:space-between;font-size:.76rem;margin-bottom:2px"><span style="color:var(--t2)">{tag}</span><span style="color:var(--yellow);font-weight:600">{round(score,1)}</span></div>',unsafe_allow_html=True)
...
def page_img_search():
    ...
    with col_up:
        ...
        st.markdown('</div>',unsafe_allow_html=True)
        st.markdown('<div class="ai-warn" style="margin-top:.85rem"><div style="font-size:.67rem;color:var(--yellow);font-weight:700;margin-bottom:2px">⚠️ Aviso</div><div style="font-size:.64rem;color:var(--t2);line-height:1.62">Análise por algoritmos computacionais. Não substitui especialistas.</div></div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    with col_res:
        if run and uploaded_img_bytes:
            ...
            if rep:
                conf_c="var(--green)" if rep["confidence"]>80 else ("var(--yellow)" if rep["confidence"]>60 else "var(--red)")
                st.markdown(f'''
                <div class="abox">
                  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:9px;margin-bottom:.5rem">
                    ...
                    <div style="background:var(--glass2);border:1px solid var(--border);border-radius:12px;padding:.55rem .9rem;text-align:center;flex-shrink:0">
                ...
                with c1: st.markdown(f'<div class="mbox"><div style="font-family:Outfit,sans-serif;font-size:.95rem;font-weight:700;color:var(--yellow)">{rep["texture"]["complexity"]}</div><div class="mlbl">Complexidade</div></div>',unsafe_allow_html=True)
                with c2: st.markdown(f'<div class="mbox"><div style="font-family:Outfit,sans-serif;font-size:.95rem;font-weight:700;color:var(--green)">{sym_lbl}</div><div class="mlbl">Simetria</div></div>',unsafe_allow_html=True)
                with c3: st.markdown(f'<div class="mbox"><div style="font-family:Outfit,sans-serif;font-size:.95rem;font-weight:700;color:var(--orange)">{rep["lines"]["direction"]}</div><div class="mlbl">Dir. Linhas</div></div>',unsafe_allow_html=True)
                l=rep["lines"]; s_img=l["strengths"]; max_s=max(s_img.values())+0.01
                st.markdown('<div class="pbox"><div style="font-family:Outfit,sans-serif;font-weight:700;font-size:.82rem;margin-bottom:.65rem;color:var(--green)">📐 Análise de Linhas</div>',unsafe_allow_html=True)
                for dir_name,val in s_img.items():
                    is_dom=dir_name==l["direction"]
                    st.markdown(f'<div style="display:flex;align-items:center;gap:7px;margin-bottom:.35rem"><span>{"★ " if is_dom else ""}{dir_name}</span><span style="font-size:.66rem;color:var(--t3);width:34px;text-align:right">{val:.1f}</span></div>',unsafe_allow_html=True)
                st.markdown(f'<div style="font-size:.68rem;color:var(--t3);margin-top:.45rem">Formas: <strong style="color:var(--green)">{" · ".join(rep["shapes"])}</strong></div></div>',unsafe_allow_html=True)
                ...
                st.markdown(f'<div class="abox"><div style="font-family:Outfit,sans-serif;font-weight:700;font-size:.82rem;margin-bottom:.72rem">🎨 Cor & Paleta</div><div style="display:flex;gap:10px;align-items:center;margin-bottom:.8rem"><div style="width:40px;height:40px;border-radius:10px;background:{hex_c};border:1.5px solid var(--border);flex-shrink:0"></div><div style="font-size:.77rem;color:var(--t2);line-height:1.72">RGB: <strong>({int(rv)},{int(gv)},{int(bv)})</strong> · {hex_c.upper()}<br>Dominante: <strong>{rep["color"]["dom"]}</strong> · {temp_str} · Sat: <strong>{rep["color"]["sat"]:.0f}%</strong></div></div><div style="display:flex;gap:4px;flex-wrap:wrap">{pal_html}</div></div>',unsafe_allow_html=True)
                if rep.get("histograms"):
                    h=rep["histograms"]; bins_x=list(range(0,256,8))[:32]
                    fig_h=go.Figure()
                    fig_h.add_trace(go.Scatter(x=bins_x,y=h["r"][:32],fill='tozeroy',name='R',line=dict(color='rgba(255,23,68,.8)',width=1.5),fillcolor='rgba(255,23,68,.10)'))
                    fig_h.add_trace(go.Scatter(x=bins_x,y=h["g"][:32],fill='tozeroy',name='G',line=dict(color='rgba(0,230,118,.8)',width=1.5),fillcolor='rgba(0,230,118,.10)'))
                    fig_h.add_trace(go.Scatter(x=bins_x,y=h["b"][:32],fill='tozeroy',name='B',line=dict(color='rgba(41,121,255,.8)',width=1.5),fillcolor='rgba(41,121,255,.10)'))
                    fig_h.update_layout(height=172,title=dict(text="Histograma RGB",font=dict(color="var(--t1)",family="Outfit",size=11)),**pc(),legend=dict(font=dict(color="var(--t3)",size=9)),margin=dict(l=10,r=10,t=32,b=8))
                    st.markdown('<div class="chart-glass">',unsafe_allow_html=True)
...
def page_chat():
    ...
    with col_c:
        ...
        for ue in st.session_state.chat_contacts:
            ...
            dot='<span class="dot-on"></span>' if online else '<span class="dot-off"></span>'
            bg="var(--glass2)" if active else "var(--glass)"; bdr="var(--border2)" if active else "var(--border)"
            st.markdown(f'<div style="background:{bg};border:1px solid {bdr};border-radius:12px;padding:8px 10px;margin-bottom:4px"><div style="display:flex;align-items:center;gap:7px">{avh(uin,30,ug)}<div style="overflow:hidden;flex:1"><div><div style="font-size:.78rem;font-weight:600;font-family:Outfit,sans-serif">{dot}{uname}</div><div style="font-size:.65rem;color:var(--t3);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{last}</div></div></div></div></div>',unsafe_allow_html=True)
    ...
    with col_m:
        if st.session_state.active_chat:
            ...
            dot='<span class="dot-on"></span>' if online else '<span class="dot-off"></span>'
            st.markdown(f'<div style="background:var(--glass);border:1px solid var(--border);border-radius:14px;padding:11px 15px;margin-bottom:.9rem;display:flex;align-items:center;gap:11px"><div style="flex-shrink:0">{avh(cin,38,cg)}</div><div style="flex:1"><div style="font-weight:700;font-size:.90rem;font-family:Outfit,sans-serif">{dot}{cname}</div><div style="font-size:.66rem;color:var(--green)">🔒 AES-256 ativo</div></div></div>',unsafe_allow_html=True)
            for msg in msgs:
                is_me=msg["from"]=="me"; cls="bme" if is_me else "bthem"
                st.markdown(f'<div style="display:flex;{"justify-content:flex-end" if is_me else ""}"><div class="{cls}">{msg["text"]}<div style="font-size:.58rem;color:var(--t3);margin-top:2px;text-align:{"right" if is_me else "left"}}">{msg["time"]}</div></div></div>',unsafe_allow_html=True)
