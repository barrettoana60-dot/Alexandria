...
def page_login():
    ...
    st.markdown('<div class="glass" style="padding:2rem 1.8rem;">', unsafe_allow_html=True)
    st.markdown('<div style="text-align:center;margin-bottom:1.8rem;"><div class="sb-logo-icon" style="width:50px;height:50px;font-size:1.3rem;margin:0 auto 10px;">🔬</div><div class="sb-logo-txt" style="font-size:1.8rem;">Nebula</div></div>', unsafe_allow_html=True)
    st.markdown('<h2 style="text-align:center;margin-bottom:1.5rem;color:var(--t0);">Bem-vindo(a) de volta!</h2>', unsafe_allow_html=True)
    ...
    if st.button("Entrar", key="btn_login", use_container_width=True):
        if email in st.session_state.users and st.session_state.users[email]["password"] == hp(password):
    ...
    st.markdown('<div style="text-align:center;margin-top:1.5rem;font-size:.8rem;color:var(--t2);">', unsafe_allow_html=True)
    st.markdown("Não tem uma conta? <a href='#' onclick='window.parent.document.querySelector(\"button[data-testid=stSidebarCollapseButton]\").click();' style='color:var(--blu);text-decoration:underline;'>Registre-se</a>", unsafe_allow_html=True)
...
def page_2fa():
    st.markdown('<div class="pw" style="max-width:400px;margin:3rem auto;">', unsafe_allow_html=True)
    st.markdown('<div class="glass" style="padding:2rem 1.8rem;">', unsafe_allow_html=True)
    st.markdown('<h2 style="text-align:center;margin-bottom:1.5rem;color:var(--t0);">Verificação em Duas Etapas</h2>', unsafe_allow_html=True)

    email = st.session_state.pending_2fa
    if not email:
        st.warning("Nenhuma tentativa de login pendente para 2FA.")
        st.session_state.page = "login"
        st.rerun()
        return

    code = st.text_input("Código de 6 dígitos", key="2fa_code", max_chars=6)

    if st.button("Verificar", key="btn_2fa_verify", use_container_width=True):
        # For demo purposes, any 6-digit code works. In a real app, this would be a generated code.
    ...
    st.markdown('<div style="text-align:center;margin-top:1.5rem;font-size:.8rem;color:var(--t2);">', unsafe_allow_html=True)
    st.markdown("Voltar ao <a href='#' onclick='window.parent.document.querySelector(\"button[data-testid=stSidebarCollapseButton]\").click();' style='color:var(--blu);text-decoration:underline;'>Login</a>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def page_register():
    ...
    st.markdown('<div class="glass" style="padding:2rem 1.8rem;">', unsafe_allow_html=True)
    st.markdown('<div style="text-align:center;margin-bottom:1.8rem;"><div class="sb-logo-icon" style="width:50px;height:50px;font-size:1.3rem;margin:0 auto 10px;">🔬</div><div class="sb-logo-txt" style="font-size:1.8rem;">Nebula</div></div>', unsafe_allow_html=True)
    st.markdown('<h2 style="text-align:center;margin-bottom:1.5rem;color:var(--t0);">Criar Nova Conta</h2>', unsafe_allow_html=True)

    new_email = st.text_input("E-mail", key="reg_email", placeholder="seu@email.com")
    new_name = st.text_input("Nome Completo", key="reg_name", placeholder="Seu Nome")
    new_area = st.text_input("Área de Pesquisa", key="reg_area", placeholder="Ex: Neurociência, IA")
    new_password = st.text_input("Senha", type="password", key="reg_password", placeholder="Mínimo 6 caracteres")
    confirm_password = st.text_input("Confirmar Senha", type="password", key="reg_confirm_password", placeholder="Repita a senha")
    ...
    st.markdown('<div style="text-align:center;margin-top:1.5rem;font-size:.8rem;color:var(--t2);">', unsafe_allow_html=True)
    st.markdown("Já tem uma conta? <a href='#' onclick='window.parent.document.querySelector(\"button[data-testid=stSidebarCollapseButton]\").click();' style='color:var(--blu);text-decoration:underline;'>Faça Login</a>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_nav():
    with st.sidebar:
        ...
        user_initials = ini(user_name)
        user_grad = ugrad(current_user_email)

        st.markdown(f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:1.5rem;padding:0 .3rem;">{avh(user_initials, 44, user_grad)}<div style="font-family:Syne,sans-serif;font-weight:700;font-size:.95rem;color:var(--t0);line-height:1.3;">{user_name}<br><span style="font-size:.65rem;font-weight:500;color:var(--t3);">{user_data.get("area", "Pesquisador")}</span></div></div>', unsafe_allow_html=True)
        ...
        for page_key, label in nav_items.items():
            active = st.session_state.page == page_key
            btn_style = f"background:var(--g3)!important;border-color:rgba(255,214,10,.2)!important;color:var(--yel)!important;-webkit-text-fill-color:var(--yel)!important;" if active else ""
            st.markdown(f'<button class="stButton" style="{btn_style}"><p>{label}</p></button>', unsafe_allow_html=True)
            if st.button(label, key=f"nav_{page_key}", use_container_width=True):
                st.session_state.page = page_key
                st.session_state.profile_view = None # Clear profile view on nav
                st.rerun()

        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown('<div class="sb-lbl">Ferramentas IA</div>', unsafe_allow_html=True)
        st.session_state.anthropic_key = st.text_input("Anthropic API Key", type="password", value=st.session_state.get("anthropic_key", ""), help="Para Claude Vision e Conexões IA (ex: sk-ant-...)")
...
def render_post(post, ctx="feed", compact=False):
    ...
    is_liked = email in post.get("liked_by", [])
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:.7rem;">
        {avh(post.get("avatar", ini(post["author"])), 38, ugrad(post["author_email"]))}
        <div style="flex:1;">
            <div style="font-family:Syne,sans-serif;font-weight:700;font-size:.9rem;color:var(--t0);line-height:1.3;">{post["author"]}</div>
            <div style="font-size:.68rem;color:var(--t3);">{post["area"]} · {time_ago(post["date"])}</div>
        </div>
        {badge(post["status"])}
    </div>
    ...
    # Title and Abstract
    st.markdown(f'<h2 style="margin-bottom:.4rem;font-size:1.05rem;">{post["title"]}</h2>', unsafe_allow_html=True)
    if not compact:
        st.markdown(f'<p style="font-size:.8rem;color:var(--t2);line-height:1.6;margin-bottom:.8rem;">{post["abstract"]}</p>', unsafe_allow_html=True)

    # Tags
    st.markdown(f'<div>{tags_html(post["tags"])}</div>', unsafe_allow_html=True)

    # Footer with actions
    if not compact:
        st.markdown(f'''
        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:.9rem;padding-top:.8rem;border-top:1px solid rgba(255,255,255,.07);">
            <div style="display:flex;align-items:center;gap:1.2rem;">
                <span style="font-size:.75rem;color:var(--t2);font-weight:500;">{post["likes"]} Curtidas</span>
                <span style="font-size:.75rem;color:var(--t2);font-weight:500;">{len(post.get("comments", []))} Comentários</span>
                <span style="font-size:.75rem;color:var(--t2);font-weight:500;">{post["views"]} Visualizações</span>
            </div>
            <div style="display:flex;gap:8px;">
                <button class="stButton" style="padding:.3rem .6rem;font-size:.7rem;border-radius:8px;" key="like_{ctx}_{post["id"]}">
                    <p>{like_icon}</p>
                </button>
                <button class="stButton" style="padding:.3rem .6rem;font-size:.7rem;border-radius:8px;" key="save_{ctx}_{post["id"]}">
                    <p>{save_icon}</p>
                </button>
            </div>
        </div>
        ...
        # Like/Save logic
        col_like, col_save, col_comment = st.columns([1,1,3])
        with col_like:
            if st.button(like_icon, key=f"btn_like_{ctx}_{post['id']}", use_container_width=True):
                if is_liked: post["liked_by"].remove(email); post["likes"]-=1
        ...
        with col_comment:
            ...
            if st.button("Comentar", key=f"btn_comment_{ctx}_{post['id']}", use_container_width=True):
                if new_comment:
                    post.setdefault("comments", []).append({"user": guser().get("name", "Anônimo"), "text": new_comment})
                    save_db(); st.rerun()
...
def render_article(article, idx, ctx="search"):
    ...
    st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.4rem;">'
                f'<a href="{article.get("url", "#")}" target="_blank" style="text-decoration: underline; color: var(--blu); font-family:Syne,sans-serif;font-weight:700;font-size:.95rem;flex:1;">{article["title"]}</a>'
                f'<span style="font-size:.65rem;color:var(--t3);flex-shrink:0;margin-left:10px;">{article.get("year", "?")}</span>'
                f'</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="font-size:.75rem;color:var(--t2);margin-bottom:.3rem;">{article.get("authors", "—")}</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="font-size:.7rem;color:var(--t3);margin-bottom:.6rem;">{article.get("source", "Web")} · {article.get("citations", 0)} citações</div>', unsafe_allow_html=True)
    st.markdown(f'<p style="font-size:.78rem;color:var(--t2);line-height:1.6;">{article.get("abstract", "Sem resumo disponível.")}</p>', unsafe_allow_html=True)

    col_save, col_view = st.columns([1, 2])
    with col_save:
        if st.button(save_icon, key=f"btn_save_article_{ctx}_{idx}", use_container_width=True):
            if is_saved:
                st.session_state.saved_articles = [a for a in st.session_state.saved_articles if a.get("url") != article.get("url")]
            else:
    ...
    with col_view:
        if article.get("url"):
            st.markdown(f'<a href="{article["url"]}" target="_blank" style="text-decoration: none;"><button class="stButton" style="width:100%;"><p>Abrir Artigo</p></button></a>', unsafe_allow_html=True)
        else:
            st.button("Link Indisponível", key=f"btn_no_link_{ctx}_{idx}", use_container_width=True, disabled=True)
...
def page_feed():
    ...
    # Compose new post
    with st.expander("✍️ Nova Publicação", expanded=st.session_state.compose_open):
        with st.form("new_post_form"):
            title = st.text_input("Título da Pesquisa", key="new_post_title")
            abstract = st.text_area("Resumo", key="new_post_abstract", height=100)
            tags_str = st.text_input("Tags (separadas por vírgula)", key="new_post_tags")

            if st.form_submit_button("Publicar", use_container_width=True):
                if title and abstract and tags_str:
                    new_id = max([p["id"] for p in st.session_state.feed_posts]) + 1 if st.session_state.feed_posts else 1
                    new_post = {
                        "id": new_id,
                        "author": user_data.get("name", "Anônimo"),
                        "author_email": email,
                        "avatar": ini(user_data.get("name", "Anônimo")),
                        "area": user_data.get("area", "Pesquisa Geral"),
                        "title": title,
                        "abstract": abstract,
                    ...
                    st.session_state.compose_open = False # Close expander
                    st.rerun()
                else:
                    st.error("Por favor, preencha todos os campos para publicar.")
    ...
    if recs:
        st.markdown('<div class="dtxt">Recomendações para você</div>', unsafe_allow_html=True)
        for p in recs:
            render_post(p, ctx="rec")
        st.markdown("<hr>", unsafe_allow_html=True)

    # Main feed
    st.markdown('<div class="dtxt">Últimas pesquisas</div>', unsafe_allow_html=True)
...
def page_search():
    st.markdown('<div class="pw">', unsafe_allow_html=True)
    st.markdown('<h1 style="padding-top:.8rem;margin-bottom:.9rem">🔍 Buscar Pesquisas</h1>', unsafe_allow_html=True)

    query = st.text_input("Termo de busca", key="search_query", placeholder="Ex: CRISPR, neurociência, IA quântica")

    if st.button("Buscar", key="btn_search", use_container_width=True) or (query and query != st.session_state.last_sq):
        st.session_state.last_sq = query
        if query:
            with st.spinner("Buscando na Nebula e na literatura científica…"):
                # Search internal posts
                internal_results = [p for p in st.session_state.feed_posts if query.lower() in p["title"].lower() or query.lower() in p["abstract"].lower() or any(query.lower() in t.lower() for t in p["tags"])]

                # Search external (Semantic Scholar & CrossRef)
                external_results_ss = search_ss(query, 5)
                external_results_cr = search_cr(query, 3)

                st.session_state.search_results = {
                    "internal": internal_results,
                    "external_ss": external_results_ss,
                    "external_cr": external_results_cr
                }
        else:
    ...
    if st.session_state.search_results:
        ...
        internal_tab, external_tab = st.tabs(["  🔬 Na Nebula  ", "  🌐 Na Web  "])

        with internal_tab:
            if st.session_state.search_results["internal"]:
                st.markdown(f'<div style="font-size:.75rem;color:var(--t3);margin-bottom:.8rem;">{len(st.session_state.search_results["internal"])} resultados encontrados na Nebula.</div>', unsafe_allow_html=True)
                for post in st.session_state.search_results["internal"]:
                    render_post(post, ctx="search_internal", compact=True)
            else:
                st.info("Nenhum resultado encontrado na Nebula para sua busca.")

        with external_tab:
            all_external_results = st.session_state.search_results["external_ss"] + st.session_state.search_results["external_cr"]
            if all_external_results:
                st.markdown(f'<div style="font-size:.75rem;color:var(--t3);margin-bottom:.8rem;">{len(all_external_results)} resultados encontrados na literatura científica.</div>', unsafe_allow_html=True)
                for idx, article in enumerate(all_external_results):
                    render_article(article, idx, ctx="search_external")
            else:
                st.info("Nenhum resultado encontrado na web para sua busca.")

    st.markdown('</div>', unsafe_allow_html=True)

def page_knowledge():
    st.markdown('<div class="pw">', unsafe_allow_html=True)
    st.markdown('<h1 style="padding-top:.8rem;margin-bottom:.9rem">📚 Artigos Salvos</h1>', unsafe_allow_html=True)

    if not st.session_state.saved_articles:
        st.markdown('<div class="glass" style="text-align:center;padding:4rem"><div style="font-size:2.2rem;opacity:.2;margin-bottom:.7rem">📚</div><div style="color:var(--t3)">Nenhum artigo salvo ainda.</div><div style="font-size:.72rem;color:var(--t3);margin-top:.4rem;">Busque por pesquisas e salve as que te interessam!</div></div>', unsafe_allow_html=True)
    else:
        for idx, article in enumerate(st.session_state.saved_articles):
            render_article(article, idx, ctx="saved")

    st.markdown('</div>', unsafe_allow_html=True)

def page_profile(profile_email):
    st.markdown('<div class="pw">', unsafe_allow_html=True)

    user_data = st.session_state.users.get(profile_email, {})
    current_user_email = st.session_state.current_user
    ...
    is_my_profile = (profile_email == current_user_email)
    is_followed = profile_email in st.session_state.followed

    # Profile Header
    st.markdown(f'''
    <div class="prof-hero">
        {avh(ini(user_data.get("name", "?")), 76, ugrad(profile_email))}
        <div style="flex:1;">
            <div style="display:flex;align-items:center;gap:8px;">
                <h1 style="margin-bottom:0;font-size:1.8rem;">{user_data.get("name", "Nome Desconhecido")}</h1>
                {f'<span class="badge-grn" style="font-size:.7rem;">✓ Verificado</span>' if user_data.get("verified") else ''}
            </div>
            <p style="font-size:.9rem;color:var(--t2);margin-top:.2rem;">{user_data.get("area", "Área não especificada")}</p>
            <p style="font-size:.8rem;color:var(--t3);margin-top:.5rem;line-height:1.6;">{user_data.get("bio", "Sem biografia.")}</p>
        </div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    # Profile Actions
    if is_my_profile:
        st.markdown('<div style="display:flex;gap:10px;margin-bottom:1rem;">', unsafe_allow_html=True)
        if st.button("✏️ Editar Perfil", key="edit_profile_btn"):
            st.session_state.page = "settings" # Redirect to settings for editing
            st.rerun()
        st.button("➕ Nova Publicação", key="new_post_profile_btn", on_click=lambda: setattr(st.session_state, 'compose_open', True))
        st.markdown('</div>', unsafe_allow_html=True)
    else:
        col1, col2, col3 = st.columns([1,1,2])
        with col1:
            if st.button("💬 Mensagem", key=f"msg_profile_{profile_email}", use_container_width=True):
                st.session_state.chat_messages.setdefault(profile_email, [])
                st.session_state.active_chat = profile_email
                st.session_state.page = "chat"
                st.rerun()
        with col2:
            follow_button_label = "✓ Seguindo" if is_followed else "+ Seguir"
            follow_button_class = "btn-grn" if is_followed else "btn-yel"
            st.markdown(f'<div class="{follow_button_class}">', unsafe_allow_html=True)
            if st.button(follow_button_label, key=f"follow_profile_{profile_email}", use_container_width=True):
                if is_followed:
                    st.session_state.followed.remove(profile_email)
                    user_data["followers"] = max(0, user_data.get("followers", 0) - 1)
                else:
                    st.session_state.followed.append(profile_email)
                    user_data["followers"] = user_data.get("followers", 0) + 1
                save_db()
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    # Stats
    st.markdown(f'''
    <div style="display:flex;gap:1.5rem;margin-bottom:1.5rem;padding:.5rem 0;">
        <div style="text-align:center;"><span style="font-family:Syne,sans-serif;font-weight:800;font-size:1.4rem;color:var(--yel);">{user_data.get("followers", 0)}</span><br><span style="font-size:.7rem;color:var(--t3);">Seguidores</span></div>
        <div style="text-align:center;"><span style="font-family:Syne,sans-serif;font-weight:800;font-size:1.4rem;color:var(--grn);">{user_data.get("following", 0)}</span><br><span style="font-size:.7rem;color:var(--t3);">Seguindo</span></div>
        <div style="text-align:center;"><span style="font-family:Syne,sans-serif;font-weight:800;font-size:1.4rem;color:var(--blu);">{len([p for p in st.session_state.feed_posts if p.get("author_email") == profile_email])}</span><br><span style="font-size:.7rem;color:var(--t3);">Publicações</span></div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    # User's Posts
    st.markdown('<h2 style="margin-bottom:.8rem;">Publicações de ' + user_data.get("name", "Este Usuário").split()[0] + '</h2>', unsafe_allow_html=True)
    user_posts = [p for p in st.session_state.feed_posts if p.get("author_email") == profile_email]
    if user_posts:
        for post in sorted(user_posts, key=lambda x: x.get("date", ""), reverse=True):
            render_post(post, ctx=f"profile_{profile_email}", compact=True)
    else:
        st.info("Nenhuma publicação encontrada para este usuário.")

    st.markdown('</div>', unsafe_allow_html=True)

def page_connections():
    st.markdown('<div class="pw">', unsafe_allow_html=True)
    st.markdown('<h1 style="padding-top:.8rem;margin-bottom:.9rem">🤝 Minhas Conexões</h1>', unsafe_allow_html=True)
    email=st.session_state.current_user; users=st.session_state.users; api_key=st.session_state.anthropic_key

    rlist=list(users.keys())
    if email in rlist: rlist.remove(email)
    rlist.insert(0,email) # Put current user at start for network viz

    n=len(rlist)
    def get_tags(ue):
        tags=set(); ud=users.get(ue,{})
        tags.update(t.lower() for t in area_tags(ud.get("area","")))
...
def page_img_search():
    ...
    with cu:
        ...
        if img_bytes and has_api:
            run_claude=st.button("🤖 Claude Vision (IA)", key="btn_vision", use_container_width=True)
        else:
            run_claude=False
  ...
  <div style="display:flex;flex-wrap:wrap;gap:4px">
    {"".join(f'<span style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:20px;padding:2px 8px;font-size:.60rem;color:var(--t3)">{k}: {v}pt</span>' for k,v in cls_["all_scores"].items())}
                ...
                with t3:
                    ...
                    st.markdown(f'<div class="ml-feat"><div style="font-size:.62rem;color:var(--t3);text-transform:uppercase;font-weight:600;margin-bottom:.45rem">Textura: <span style="color:var(--yel)">{tt}</span></div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;font-size:.72rem"><div><div style="color:var(--t2)">Contraste</div><strong style="color:var(--t0)">{contrast:.4f}</strong></div><div><div style="color:var(--t2)">Homog.</div><strong style="color:var(--t0)">{hom:.4f}</strong></div><div><div style="color:var(--t2)">Correlação</div><strong style="color:var(--t0)">{corr:.4f}</strong></div></div></div>', unsafe_allow_html=True)
                ...
                with t5:
                    ...
                    st.markdown(f'<div class="ml-feat"><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;font-size:.72rem"><div><div style="color:var(--t2)">Score periódico</div><strong style="color:{c_fft}">{per:.1f}</strong></div><div><div style="color:var(--t2)">Escala dominante</div><strong style="color:var(--yel)">{dom}</strong></div><div><div style="color:var(--t2)">Estrutura</div><strong style="color:var(--blu)">{"Periódica ✓" if is_per else "Não-periódica"}</strong></div></div></div>', unsafe_allow_html=True)
        ...
        if st.session_state.active_chat:
            ...
            with cb2:
                if st.button("→", key=f"ms_{contact}", use_container_width=True):
                    if nm: now=datetime.now().strftime("%Y-%m-%d %H:%M:%S"); st.session_state.chat_messages.setdefault(contact,[]).append({"from":"me","text":nm,"time":now}); st.rerun()
        else:
...
def main():
        ...
        page_profile(st.session_state.profile_view); return
    {
        ...
        "chat": page_chat, "connections": page_connections, "settings": page_settings,
