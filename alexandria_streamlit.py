  ...
  "sugestoes": [
    {{"email": "<email>", "razao": "<explicação científica de 1-2 frases>", "score": <0-100>, "temas_comuns": ["<tema1>", "<tema2>"]}}
        ...
        if resp.status_code == 200:
            ...
            # The r-string literal needs to be properly closed.
            # Using a raw string r'```' ensures the backticks are treated literally.
  ...
  "sugestoes": [
    {{"email": "<email>", "razao": "<explicação científica de 1-2 frases>", "score": <0-100>, "temas_comuns": ["<tema1>", "<tema2>"]}}
        ...
        if resp.status_code == 200:
            ...
            # Using a raw string r'```' ensures the backticks are treated literally.
            text = re.sub(r'^```json\s*', '', text, flags=re.DOTALL)
            text = re.sub(r'\s*```INNERCHAT_CB_aiwv6vh9m#x27;, '', text, flags=re.DOTALL)
...
def render_nav():
    ...
    with st.sidebar:
        ...
        for i, (key, label, col) in enumerate(NAV):
            ...
            if is_a:
                colors = {"yel":"#FFD60A","grn":"#06D6A0","blu":"#4CC9F0",
                          "red":"#FF3B5C","pur":"#B17DFF","orn":"#FF8C42"}
                c = colors.get(col, "#FFD60A")
...
def render_post(post,ctx="feed",show_author=True,compact=False):
    ...
    if show_author:
        ...
        hdr=f'<div style="padding:.35rem 1.1rem .15rem;display:flex;justify-content:space-between;align-items:center"><span></span><span style="color:var(--t3);font-size:.63rem">{dt}</span>{badge(post["status"])}</div>'
    ...
    if st.session_state.get(f"cmt_{ctx}_{pid}",False):
        for c in post.get("comments",[]):
            ...
            st.markdown(f'<div class="cmt"><div><div style="display:flex;align-items:center;gap:7px;margin-bottom:.2rem">{avh(ci,26,cg)}<span style="font-size:.73rem;font-weight:700;color:var(--yel)">{c["user"]}</span></div></div><div style="font-size:.78rem;color:var(--t2);line-height:1.55;padding-left:33px">{c["text"]}</div></div>', unsafe_allow_html=True)
...
def page_feed():
    ...
    with cs:
        ...
        for i,(t,c) in enumerate([("Quantum ML","34"),("CRISPR 2026","28"),("Neuroplasticidade","22"),("LLMs Científicos","19"),("Matéria Escura","15")]):
            st.markdown(f'<div style="padding:.32rem 0;border-bottom:1px solid rgba(255,255,255,.04)"><div><div style="font-size:.57rem;color:var(--t3)">#{i+1}</div><div style="font-size:.76rem;font-weight:600;color:{VIB[i]}">{t}</div><div style="font-size:.58rem;color:var(--t3)">{c} pesquisas</div></div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
        ...
        if run_claude and img_bytes:
            ...
            if ai_err:
                ...
                try:
                    ...
                    clean = re.sub(r'\s*```INNERCHAT_CB_aiwv6vh9m#x27;, '', clean, flags=re.DOTALL)
