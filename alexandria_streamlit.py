...
GRAD_POOL=[
    "135deg,#0A6EBD,#6A9C89","135deg,#1F5E9E,#7D9D8A","135deg,#164863,#5F8B7A",
    "135deg,#0B3954,#4F7773","135deg,#0A2472,#597E7A","135deg,#102B3F,#4D7269",
    "135deg,#0B1E33,#3F5E56","135deg,#0D2135,#355249",
]
...
def init():
    ...
    st.session_state.setdefault("ml_cache",{})  # for image ML results
...
# ════════════════════════════════════════════════
#  REAL ML IMAGE ANALYSIS PIPELINE (CACHED)
# ════════════════════════════════════════════════
...
def run_full_ml_pipeline_cached(img_bytes):
    """Cached version of full ML analysis pipeline."""
...
def run_full_ml_pipeline(img_bytes):
    ...
    try:
        ...
        w, h = img.size
        scale = min(384/w, 384/h)  # reduced from 512 to 384 for speed
...
VIB=["#0A6EBD","#1F5E9E","#164863","#0B3954","#0A2472","#102B3F","#0B1E33","#0D2135","#1A2A4A","#234567"]
...
def avh(initials,sz=40,grad=None):
    fs=max(sz//3,9); bg=grad or "linear-gradient(135deg,#0A6EBD,#6A9C89)"
...
def page_knowledge():
    ...
    for e1,e2,common,strength in edges:
        ...
        fig.add_trace(go.Scatter3d(x=[p1["x"],p2["x"],None],y=[p1["y"],p2["y"],None],z=[p1["z"],p2["z"],None],
            mode="lines",line=dict(color=f"rgba(10,110,189,{alpha:.2f})",width=min(3,1+strength)),hoverinfo="none",showlegend=False))
            ...
            if analyses:
                for f,an in analyses.items():
                    with st.expander(f"🔬 {f}"):
                        ...
                        if topics:
                            fig2=go.Figure(go.Pie(labels=list(topics.keys()),values=list(topics.values()),hole=0.5,marker=dict(colors=VIB[:len(topics)],line=dict(color=["#0B0F1A"]*15,width=2)),textfont=dict(color="white",size=8)))
...
def page_analytics():
    ...
    with tpr:
        ...
        if prefs:
            ...
            if len(cats)>=3:
                fig3=go.Figure(go.Scatterpolar(r=vals+[vals[0]],theta=cats+[cats[0]],fill='toself',line=dict(color="#0A6EBD"),fillcolor="rgba(10,110,189,.10)"))
...
# ═══════════════════════════════════════════════
#  REAL AI IMAGE ANALYSIS PAGE (with Caching)
# ═══════════════════════════════════════════════
def page_img_search():
    st.markdown('<div class="pw">', unsafe_allow_html=True)
    ...
    with cr:
        if run and img_bytes:
            # Generate cache key based on image bytes
  ...
  <div style="display:flex;flex-wrap:wrap;gap:4px">
    {"".join(f'<span style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:20px;padding:2px 8px;font-size:.60rem;color:var(--t3)">{k}: {v}pt</span>' for k,v in cls_["all_scores"].items())}
                ...
                with t1:
                    ...
                    fig_e=go.Figure(go.Bar(y=eh,marker=dict(color=list(range(16)),colorscale=[[0,"#1A1B2E"],[.3,"#FF3B5C"],[.7,"#0A6EBD"],[1,"#6A9C89"]]),x=list(range(len(eh)))))
                ...
                with t3:
                    ...
                    if glcm_props:
                        ...
                        fig_gl=go.Figure()
                        fig_gl.add_trace(go.Bar(x=names_g,y=vals_g,marker=dict(color=vals_norm,colorscale=[[0,"#1A1B2E"],[.4,"#4CC9F0"],[.7,"#6A9C89"],[1,"#0A6EBD"]]),text=[f"{v:.4f}" for v in vals_g],textposition="outside",textfont=dict(color="#6B6F88",size=8)))
                ...
                with t6:
                    ...
                    if h_data:
                        ...
                        fig4.add_trace(go.Scatter(x=bx,y=h_data.get("g",[])[:32],fill='tozeroy',name='G',line=dict(color='rgba(106,156,137,.9)',width=1.5),fillcolor='rgba(106,156,137,.10)'))
