...
SEED_POSTS = [
    {"id": 1, "author": "Carlos Mendez", "author_email": "carlos@nebula.ai", "avatar": "CM", "area": "Neurociência",
     ...
     "comments": [{"user": "Maria Silva", "text": "Excelente metodologia!"}], "status": "Em andamento",
     "date": "2026-02-10", "liked_by": [], "saved_by": [], "connections": ["sono", "memória"], "views": 312},
    ...
    {"id": 3, "author": "Rafael Souza", "author_email": "rafael@nebula.ai", "avatar": "RS", "area": "Computação",
     ...
     "tags": ["quantum ML", "otimização", "TSP"], "likes": 201, "comments": [], "status": "Em andamento",
    ...
    {"id": 4, "author": "Priya Nair", "author_email": "priya@nebula.ai", "avatar": "PN", "area": "Astrofísica",
     ...
     "status": "Publicado", "date": "2026-02-01", "liked_by": [], "saved_by": [], "connections": ["cosmologia"],
     "views": 2180},
    {"id": 5, "author": "João Lima", "author_email": "joao@nebula.ai", "avatar": "JL", "area": "Psicologia",
     ...
     "comments": [{"user": "Carlos M.", "text": "Muito relevante!"}], "status": "Publicado",
...
CHAT_INIT = {
    "carlos@nebula.ai": [{"from": "carlos@nebula.ai", "text": "Oi! Vi seu comentário na pesquisa.", "time": "09:14"},
...
def orb_keypoints(gray_uint8):
    ...
    try:
        ...
        return {
            ...
            "mean_scale": 1.0,
            "distribution": "uniforme" if len(kp) > 5 and np.std(np.array(kp)[:, 0]) / (
                        np.std(np.array(kp)[:, 1]) + 1e-5) < 1.5 else "concentrado"
        }
    except Exception:
        return {"keypoints": np.zeros((0, 2)), "n_keypoints": 0, "cluster_centers": [], "scales": [], "mean_scale": 1.0,
                "distribution": "n/a"}
...
def run_full_ml_pipeline(img_bytes):
    ...
    try:
        ...
        orig_size = img.size
        w, h = img.size
        scale = min(384 / w, 384 / h)  # resolução reduzida para velocidade
...
def topic_dist(kws):
    tm = {
        ...
        "Computação & IA": ["algoritmo", "machine", "learning", "inteligência", "dados", "computação", "ia", "deep",
                            "quantum"],
...
def analyze_doc(fname, fbytes, ftype, area=""):
    ...
    if text:
        ...
        r["writing_quality"] = min(100, 50 + (15 if len(r["keywords"]) > 15 else 0) + (15 if words > 1000 else 0) + (
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for p in r.json().get("data", []):
                ...
                link = pdf.get("url", "") or (f"https://arxiv.org/abs/{arx}" if arx else (
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for p in r.json().get("message", {}).get("items", []):
                ...
                ars = p.get("author", []) or []
                au = ", ".join(
                    f'{a.get("given", "").split()[0] if a.get("given") else ""} {a.get("family", "")}'.strip() for a in
                    ars[:3])
                if len(ars) > 3:
                    au += " et al."
...
def get_recs(email, n=2):
    ...
    def sc(p):
        return sum(pr.get(t.lower(), 0) for t in p.get("tags", []) + p.get("connections", []))
...
EMAP = {"pdf": "PDF", "docx": "Word", "xlsx": "Planilha", "csv": "Dados", "txt": "Texto", "py": "Código",
        "md": "Markdown", "png": "Imagem", "jpg": "Imagem", "jpeg": "Imagem"}
...
:root {
  --bg:#0B0F1A; --bg2:#121724; --bg3:#1A2130;
...
.stApp::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 60% 50% at -5% 0%,rgba(10,110,189,.07) 0%,transparent 60%),radial-gradient(ellipse 50% 40% at 105% 0%,rgba(76,201,240,.07) 0%,transparent 55%),radial-gradient(ellipse 40% 50% at 50% 110%,rgba(106,156,137,.06) 0%,transparent 60%);}
...
.sb-logo-icon{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#0A6EBD,#6A9C89);display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0;}
.sb-logo-txt{font-family:'Syne',sans-serif;font-weight:900;font-size:1.2rem;letter-spacing:-.04em;background:linear-gradient(135deg,#0A6EBD,#6A9C89);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
...
.stTextInput input:focus,.stTextArea textarea:focus{border-color:rgba(10,110,189,.4)!important;box-shadow:0 0 0 3px rgba(10,110,189,.08)!important;}
...
.bme{background:linear-gradient(135deg,rgba(10,110,189,.15),rgba(106,156,137,.1));border:1px solid rgba(10,110,189,.2);border-radius:18px 18px 4px 18px;padding:.55rem .88rem;max-width:70%;margin-left:auto;margin-bottom:5px;font-size:.82rem;line-height:1.6;}
...
.stTabs [aria-selected="true"]{background:var(--g3)!important;color:var(--yel)!important;border:1px solid rgba(10,110,189,.2)!important;font-weight:700!important;}
...
.ai-card{background:linear-gradient(135deg,rgba(10,110,189,.06),rgba(106,156,137,.04));border:1px solid rgba(10,110,189,.18);border-radius:var(--r16);padding:1rem;margin-bottom:.6rem;}
...
def page_login():
  ...
  <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:.8rem">
    <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#0A6EBD,#6A9C89);display:flex;align-items:center;justify-content:center;font-size:1.4rem;box-shadow:0 0 24px rgba(10,110,189,.3)">🔬</div>
    <div style="font-family:Syne,sans-serif;font-size:2.6rem;font-weight:900;letter-spacing:-.06em;background:linear-gradient(135deg,#0A6EBD,#6A9C89);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">Nebula</div>
  </div>
...
def render_nav():
    ...
    with st.sidebar:
        ...
        for i, (key, label, col) in enumerate(NAV):
            ...
            if is_a:
                colors = {"yel": "#0A6EBD", "grn": "#6A9C89", "blu": "#4CC9F0",
                ...
                active_styles += (
                    ...
                    f'background:rgba(255,255,255,.14)!important;'
        ...
        if ak and ak.startswith("sk-"):
            st.markdown('<div style="font-size:.55rem;color:#6A9C89;padding:.1rem .2rem">● Claude Vision ativo</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="font-size:.55rem;color:#555770;padding:.1rem .2rem">● Insira chave para IA</div>', unsafe_allow_html=True)
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown(f'<div style="display:flex;align-items:center;gap:8px;padding:.2rem .1rem">{avh(ini_, 32, g)}<div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:.78rem;color:#FFF;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px">{name}</div><div style="font-size:.58rem;color:#6B6F88">{u.get("area", "")[:18]}</div></div></div>', unsafe_allow_html=True)
  ...
  <div style="flex:1">
    ...
    if not is_me:
        ...
        with tp:
            ...
            if not user_posts:
                st.markdown('<div class="glass" style="padding:2rem;text-align:center;color:var(--t3)">Nenhuma pesquisa publicada.</div>', unsafe_allow_html=True)
        with tl:
            for p in sorted(liked_posts, key=lambda x: x.get("date", ""), reverse=True):
                render_post(p, ctx="prof_liked", compact=True)
            if not liked_posts:
                st.markdown('<div class="glass" style="padding:2rem;text-align:center;color:var(--t3)">Nenhuma curtida.</div>', unsafe_allow_html=True)
    else:
        saved_arts = st.session_state.saved_articles
        tm, tl, ts2, ts = st.tabs(["  ✏️ Meus Dados  ", f"  📝 Publicações ({len(user_posts)})  ",
                                    f"  ❤️ Curtidas ({len(liked_posts)})  ", f"  🔖 Salvos ({len(saved_arts)})  "])
        with tm:
        ...
        with tl:
            if user_posts:
                ...
                st.markdown('<div class="glass" style="padding:2.5rem;text-align:center;color:var(--t3)">Nenhuma pesquisa ainda.</div>', unsafe_allow_html=True)
        with ts2:
            if liked_posts:
                for p in sorted(liked_posts, key=lambda x: x.get("date", ""), reverse=True):
                    render_post(p, ctx="mylk", compact=True)
            else:
                st.markdown('<div class="glass" style="padding:2.5rem;text-align:center;color:var(--t3)">Nenhuma curtida ainda.</div>', unsafe_allow_html=True)
        with ts:
            if saved_arts:
                for idx, a in enumerate(saved_arts):
                    ...
                    if st.button("🗑 Remover", key=f"rm_sa_{uid2}", use_container_width=True):
                        st.session_state.saved_articles = [s for s in st.session_state.saved_articles if
                                                            s.get('doi') != a.get('doi')]
                        save_db()
                        st.rerun()
            else:
...
def render_post(post, ctx="feed", show_author=True, compact=False):
    ...
    if st.session_state.get(f"cmt_{ctx}_{pid}", False):
        for c in post.get("comments", []):
            ci = ini(c["user"])
            ce2 = next((e for e, u in st.session_state.users.items() if u.get("name") == c["user"]), "")
            cg = ugrad(ce2)
            st.markdown(
...
def page_feed():
    ...
    with cm:
        if co:
            ...
            with c1c:
                ntg = st.text_input("Tags (vírgula)", key="np_tg", placeholder="neurociência, IA")
            with c2c:
                nst = st.selectbox("Status", ["Em andamento", "Publicado", "Concluído"], key="np_st")
            ...
            with cp:
                if st.button("🚀 Publicar", key="btn_pub", use_container_width=True):
                    if not nt or not nab:
                        ...
                        np2 = {"id": len(st.session_state.feed_posts) + 200 + hash(nt) % 99, "author": uname,
                               "author_email": email, "avatar": uin, "area": u.get("area", ""), "title": nt,
                               "abstract": nab, "tags": tags, "likes": 0, "comments": [], "status": nst,
                        ...
                        st.rerun()
            with cc:
                if st.button("✕ Cancelar", key="btn_cc", use_container_width=True):
                    st.session_state.compose_open = False
                    st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        else:
            ...
            with ac:
                st.markdown(f'<div style="padding-top:6px">{avh(uin, 38, g)}</div>', unsafe_allow_html=True)
            with bc:
                if st.button(f"No que está pesquisando, {uname.split()[0]}?", key="oc", use_container_width=True):
                    st.session_state.compose_open = True
                    st.rerun()
        ff = st.radio("", ["🌐 Todos", "👥 Seguidos", "🔖 Salvos", "🔥 Populares"], horizontal=True, key="ff",
    ...
    with cs:
        ...
        for ue, ud in list(users.items()):
            if ue == email or sn >= 5:
                continue
            rn = ud.get("name", "?")
            if sq and sq.lower() not in rn.lower() and sq.lower() not in ud.get("area", "").lower():
                continue
            sn += 1
            ...
            with cf2:
                ...
                st.markdown('</div>', unsafe_allow_html=True)
            with cv2:
                if st.button("👤 Ver", key=f"svr_{ue}", use_container_width=True):
        ...
        for i, (t, c) in enumerate([("Quantum ML", "34"), ("CRISPR 2026", "28"), ("Neuroplasticidade", "22"),
                                     ("LLMs Científicos", "19"), ("Matéria Escura", "15")]):
            st.markdown(
        ...
        if st.button("🔍 Buscar", key="btn_s", use_container_width=True):
            if q:
                with st.spinner("Buscando…"):
                    nr = [p for p in st.session_state.feed_posts if
                          q.lower() in p["title"].lower() or q.lower() in p["abstract"].lower()]
                    sr = search_ss(q, 6)
                    cr = search_cr(q, 3)
                    st.session_state.search_results = {"nebula": nr, "ss": sr, "cr": cr}
    ...
    if st.session_state.get("search_results") and st.session_state.get("last_sq"):
        ...
        with ta:
            if neb:
                ...
                for p in neb:
                    render_post(p, ctx="srch_all", compact=True)
            if web:
                if neb:
                    st.markdown('<hr>', unsafe_allow_html=True)
        ...
        with tn:
            for p in neb:
                render_post(p, ctx="srch_neb", compact=True)
            if not neb:
                st.info("Nenhuma pesquisa.")
        with tw:
            for idx, a in enumerate(web):
                render_article(a, idx=idx, ctx="web_t")
            if not web:
                st.info("Nenhum artigo.")
    st.markdown('</div>', unsafe_allow_html=True)
...
def page_knowledge():
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode="markers+text", marker=dict(size=nsizes, color=ncolors, opacity=.9,
                                         line=dict(color="rgba(255,255,255,.08)", width=1.5)),
    ...
    for col, (cls, v, l) in zip([c1, c2, c3, c4],
                                 [("mval-yel", len(rlist), "Pesquisadores"), ("mval-grn", len(edges), "Conexões"),
                                  ...
                                  ("mval-red", len(st.session_state.feed_posts), "Pesquisas")]):
        with col:
        ...
        if not api_key or not api_key.startswith("sk-"):
            ...
            if ai_result:
                ...
                for sug in suggestions:
                    ...
                    score_col = "#6A9C89" if score >= 80 else ("#0A6EBD" if score >= 60 else "#FF8C42")
  ...
  <div>{tags_html(temas[:5])}</div>
    ...
    with tall:
        ...
        for ue, ud in users.items():
            ...
            rn = ud.get("name", "?")
            ua = ud.get("area", "")
            if sq2 and sq2.lower() not in rn.lower() and sq2.lower() not in ua.lower():
                continue
            is_fol = ue in st.session_state.followed
            rg = ugrad(ue)
            st.markdown(
...
def page_folders():
    ...
    if st.button("📁 Criar", key="btn_nf", use_container_width=True):
        if nfn.strip():
            if nfn not in st.session_state.folders:
                st.session_state.folders[nfn] = {"desc": nfd, "files": [], "notes": "", "analyses": {}}
                save_db()
    ...
    for fn, fd in list(st.session_state.folders.items()):
        if not isinstance(fd, dict):
            fd = {"files": fd, "desc": "", "notes": "", "analyses": {}}
        ...
        with st.expander(f"📁 {fn} — {len(files)} arquivo(s)"):
            ...
            if up:
                ...
                fd["files"] = files
                save_db()
                st.success(f"{len(up)} adicionado(s)!")
            if files:
                for f in files:
                    ft = ftype(f)
                    ha = f in analyses
                    icon = {"PDF": "📄", "Word": "📝", "Planilha": "📊", "Dados": "📈", "Código": "🐍", "Imagem": "🖼",
                            "Markdown": "📋"}.get(ft, "📄")
...
def page_analytics():
    ...
    with tf:
        folders = st.session_state.folders
        if not folders:
            st.markdown('<div class="glass" style="text-align:center;padding:3rem;color:var(--t3)">Crie pastas.</div>', unsafe_allow_html=True)
        else:
            ...
            for col, (cls, v, l) in zip([c1, c2, c3, c4],
                ...
                with col:
                    st.markdown(f'<div class="mbox"><div class="{cls}">{v}</div><div class="mlbl">{l}</div></div>', unsafe_allow_html=True)
            if all_top:
                fig = go.Figure(go.Bar(x=list(all_top.values())[:8], y=list(all_top.keys())[:8], orientation='h',
                                        marker=dict(color=VIB[:8])))
                fig.update_layout(**{**pc_dark(), 'height': 250,
                                      'yaxis': dict(showgrid=False, color="#6B6F88", tickfont=dict(size=9)),
                                      'title': dict(text="Temas", font=dict(color="#E8E9F0", family="Syne", size=11))})
                st.markdown('<div class="chart-wrap">', unsafe_allow_html=True)
                st.plotly_chart(fig, use_container_width=True)
                st.markdown('</div>', unsafe_allow_html=True)
    with tp:
        ...
        if not my_posts:
            st.markdown('<div class="glass" style="text-align:center;padding:2.5rem;color:var(--t3)">Publique pesquisas.</div>', unsafe_allow_html=True)
        else:
            c1, c2, c3 = st.columns(3)
            with c1:
                st.markdown(f'<div class="mbox"><div class="mval-yel">{len(my_posts)}</div><div class="mlbl">Pesquisas</div></div>', unsafe_allow_html=True)
            with c2:
                st.markdown(f'<div class="mbox"><div class="mval-grn">{sum(p["likes"] for p in my_posts)}</div><div class="mlbl">Curtidas</div></div>', unsafe_allow_html=True)
            with c3:
                st.markdown(f'<div class="mbox"><div class="mval-blu">{sum(len(p.get("comments", [])) for p in my_posts)}</div><div class="mlbl">Comentários</div></div>', unsafe_allow_html=True)
    ...
    with ti:
        ...
        nn = st.text_area("Notas", value=d.get("notes", ""), key="e_nt", height=70)
        if st.button("💾 Salvar", key="btn_sm"):
    ...
    with tpr:
        ...
        if prefs:
            ...
            st.info("Interaja com pesquisas.")
    st.markdown('</div>', unsafe_allow_html=True)
...
def page_img_search():
                ...
                with t4:
                    ...
                    if pal:
                        ...
                        for ci, cp in enumerate(pal):
                            ...
                            st.markdown(
                                f'<div style="display:flex;align-items:center;gap:9px;margin-bottom:.4rem"><div style="width:30px;height:30px;border-radius:7px;background:{hex_c};border:1.5px solid rgba(255,255,255,.08);flex-shrink:0"></div><div style="flex:1"><div style="display:flex;justify-content:space-between;font-size:.70rem;color:var(--t2);margin-bottom:2px"><span style="font-weight:600">{hex_c.upper()}</span><span>{pct:.1f}%</span></div>{bar}</div><div style="font-size:.60rem;color:var(--t3);width:80px;text-align:right">RGB({r},{g},{b})</div></div>',
                                unsafe_allow_html=True)
  ...
  {f'<div style="background:rgba(106,156,137,.05);border:1px solid rgba(106,156,137,.12);border-radius:8px;padding:.5rem .7rem;font-size:.72rem;color:var(--t2);line-height:1.65"><strong style="color:var(--grn)">💡 Observações:</strong> {obs}</div>' if obs else ""}
</div>''', unsafe_allow_html=True)
                    if termos:
        ...
        if ml_r and ml_r.get("ok") and not (run or run_claude):
            ...
            with tn2:
                ...
                if not nr:
                    st.markdown('<div style="color:var(--t3);padding:.8rem">Nenhuma pesquisa similar.</div>', unsafe_allow_html=True)
            with tf2:
                fm = []
                ...
                if not fm:
                    st.markdown('<div style="color:var(--t3);padding:.8rem">Nenhuma pasta relacionada.</div>', unsafe_allow_html=True)
            with tw2:
                ck = f"img_{kw_s[:40]}"
                ...
                if not wr2:
                    st.markdown('<div style="color:var(--t3);padding:.8rem">Sem resultados.</div>', unsafe_allow_html=True)
